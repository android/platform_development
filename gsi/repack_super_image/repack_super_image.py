...
"""This script is for test infrastructure to mix super image."""
...
def partition_image(string):
  """Split a string into a pair of strings by "="."""
  part, _, img = string.partition("=")
  if not part or not img:
    raise ValueError(string + " is not in the format of "
                     "PARITITON_NAME=IMAGE_PATH.")
  return part, existing_abs_path(img)
...
def rewrite_misc_info(part_img_dict, lpmake_path, input_file, output_file):
  ...
  for partition_name in partition_names:
    ...
    output_file.write("%s_image=%s\n" %
                      (partition_name, part_img_dict[partition_name]))
...
def main():
  parser = argparse.ArgumentParser(
    description="This script is for test infrastructure to add images to a "
                "super image.")
  ...
  parser.add_argument("--super-img",
                      required=True,
                      type=existing_abs_path,
                      help="The path to the super image.")
  parser.add_argument("output_super_img",
                      metavar="OUTPUT_SUPER_IMG",
                      type=os.path.abspath,
                      help="The path to the output super image. It can be "
                           "identical to the input super image path.")
  ...
  try:
    ...
    if not is_sparse_image(args.super_img):
      ...
      with tempfile.NamedTemporaryFile(
        ...
        subprocess.check_call([simg2img_path, args.super_img,
                               super_img_path])
