...
class RunCommandTest(unittest.TestCase):
  ...
  def test_error_logging_stdout(self):
    error_cmd = 'echo foo; echo bar; exit 1'
  ...
  def test_error_logging_stderr(self):
    error_cmd = '(echo 123; echo 456;)>&2; exit 3'
    cmd_utils.run_command(error_cmd, shell=True, raise_on_error=False,
                          read_stderr=True)
    self.assertEqual(('ERROR', 'Failed to execute command: %r' % error_cmd),
                     self._log_entries[0])
    self.assertEqual(('ERROR', '  Return code: 3'), self._log_entries[1])
    self.assertEqual(('ERROR', "  stderr: '123\\n456\\n'"),
                     self._log_entries[2])

  def test_error_logging_stdout_stderr(self):
    error_cmd = 'echo foo; echo bar; (echo 123; echo 456;)>&2; exit 5'
    cmd_utils.run_command(error_cmd, shell=True, raise_on_error=False,
                          read_stdout=True, read_stderr=True)
    self.assertEqual(('ERROR', 'Failed to execute command: %r' % error_cmd),
                     self._log_entries[0])
    self.assertEqual(('ERROR', '  Return code: 5'), self._log_entries[1])
    self.assertEqual(('ERROR', "  stdout: 'foo\\nbar\\n'"),
                     self._log_entries[2])
    self.assertEqual(('ERROR', "  stderr: '123\\n456\\n'"),
                     self._log_entries[3])
