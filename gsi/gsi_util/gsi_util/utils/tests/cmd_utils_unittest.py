...
class RunCommandTest(unittest.TestCase):
  ...
  def test_log_stdout(self):
    ...
    self.assertEqual((0, None, None), result)
    self.assertEqual(('INFO',
                      "Command executed successfully: 'echo 456'"),
                     self._log_entries[0])
    self.assertEqual(('INFO', "  stdout: '456\\n'"),
                     self._log_entries[1])

  def test_log_stderr_on_error(self):
  ...
  def test_log_stdout_and_log_stderr_on_error(self):
    error_cmd = 'echo foo; echo bar; (echo 123; echo 456;)>&2; exit 5'
    result = cmd_utils.run_command(error_cmd, shell=True, raise_on_error=False,
                                   log_stdout=True, log_stderr_on_error=True)
