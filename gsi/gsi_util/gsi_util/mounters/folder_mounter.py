...
class _FolderFileAccessor(base_mounter.BaseFileAccessor):
  ...
  def _handle_prepare_file(self, file_in_storage):
    filename = os.path.join(self._folder_dir, file_in_storage)
    logging.debug('Prepare file %s -> %s', file_in_storage, filename)
    if not os.path.isfile(filename):
      logging.error('File is not exist: %s', file_in_storage)
      return None
