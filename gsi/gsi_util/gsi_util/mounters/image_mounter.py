...
class _ImageFileAccessor(base_mounter.BaseFileAccessor):
  ...
  def _handle_prepare_file(self, file_in_storage):
    mount_filename = os.path.join(self._mount_point, file_in_storage)
    filename = os.path.join(self._temp_dir, file_in_storage)
    logging.info('Prepare file %s -> %s', file_in_storage, filename)
    if not os.path.isfile(mount_filename):
      logging.error('File does not exist: %s', file_in_storage)
      return None
    ...
    return base_mounter.MounterFile(filename, cleanup)
