...
import gsi_util.utils.debugfs as debugfs
...
class ImageMounter(base_mounter.BaseMounter):
  ...
  def _detect_system_as_root(cls, raw_image_file):
    ...
    for filename in cls._SYSTEM_FILES:
      filespec = os.path.join('/', filename)
      if debugfs.get_type(raw_image_file, filespec) != 'regular':
        system_without_root = False
        break

    system_as_root = True
    for filename in cls._SYSTEM_FILES:
      filespec = os.path.join('/system', filename)
      if debugfs.get_type(raw_image_file, filespec) != 'regular':
