...
def _dump_list(dump_list, file_accessor):
  ...
  for dumper in dumper_set:
    ...
    with dumper_class(file_accessor, dumper_args) as dumper_instance:
      ...
      for info in info_list_for_the_dumper:
        dump_id = info[0]
...
def do_dump(args):
  ...
  info_list = _make_info_list(args.ID) if len(args.ID) else INFO_LIST
  with mounter as file_accessor:
    report = _dump_list(info_list, file_accessor)
