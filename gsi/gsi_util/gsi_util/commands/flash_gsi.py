...
def do_flash_gsi(args):
  ...
  cmd_utils.fastboot_erase()  # erases userdata/cache partition
  cmd_utils.fastboot_erase('system')  # erases system partition
  # Not every device has metadata partition, so allow_error is True.
  cmd_utils.fastboot_erase('metadata', allow_error=True)

  # Flashes GSI.
  cmd_utils.fastboot_flash('system', args.image)
  ...
  with file_utils.UnopenedTemporaryFile() as vbmeta_image:
    ...
    cmd_utils.fastboot_flash('vbmeta', vbmeta_image, allow_error=True)

  # Reboots the device.
  cmd_utils.fastboot_reboot()
...
def setup_command_args(subparsers):
  ...
  parser = subparsers.add_parser(
      'flash_gsi', help=('Flash a GSI image - '
                         ...
                         'and erase metadata partition (if the device has).'))
