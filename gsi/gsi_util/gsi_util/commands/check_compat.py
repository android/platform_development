...
class CheckReporter(object):
  ...
  def output(self, check_result):
    summary_result = True
    for result_item in check_result:
      result = self._output_result_item(result_item)
      summary_result = summary_result and result
    self._output_summary(summary_result)
...
DUMP_DESCRIPTION = """'check_compat' command dumps information from given image
