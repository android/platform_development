...
"""Runs Treble SEPolicy files merge between /system and /vendor.

SEPolic files have been split into two two parts: platform V.S. non-platform
...
When device boots a GSI, /init will merge SEPolicy files from those partitions.
If the merge fails, device will reboot to fastboot mode. We can do the same
SEPolicy merge on the host side to catch this error, prior to use a GSI.
...
class SepolicyChecker(object):   # pylint: disable=too-few-public-methods
  ...
  def _get_kernel_policy_version(self):
    ...
    with xml_dumper.XmlDumper(
        self._file_accessor,
        ('/system/compatibility_matrix.xml',)) as dumper_instance:
  ...
  def check(self):
    ...
    merge_cmd = ['secilc', '-m', '-M', 'true', '-G', '-N',
                 '-c', policy_version, '-o', '/dev/null', '-f', '/dev/null']

    # Selects the mapping file based on vendor-used platform version.
    vendor_plat_vers = self._get_vendor_mapping_version()
    mapping_sepolicy_cil = ('/system/etc/selinux/mapping/' +
                            vendor_plat_vers + '.cil')
    ...
    with self._file_accessor.prepare_multi_files([
      ...
      merge_cmd += [plat_sepolicy, mapping_sepolicy]
      ...
      if odm_sepolicy:   # odm_sepolicy is optional.
        merge_cmd += [odm_sepolicy]

      # Executes the merge command.
      return_code, _, stderr = cmd_utils.run_command(
          merge_cmd, raise_on_error=False, log_stdout=True,
          log_stderr=True, read_stderr=True)
