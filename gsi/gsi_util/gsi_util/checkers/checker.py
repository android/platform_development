...
# list to the caller.
_CHECK_LIST = (
    CheckListItem('vintf', VintfChecker),
)
...
class Checker(object):
  ...
  def make_check_list(check_items):
    ...
    for check_item in check_items:
      ...
      assert matched_items, 'Unknown check item: {!r}'.format(check_item)
      # Checks there is exactly one match.
      assert len(matched_items) == 1, (
          'Duplicated check items: {!r} in the check list'.format(check_item))
