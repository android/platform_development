...
public class DemoModeController extends Activity implements OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (mControllingColor) {
            float hue = y / (h / 360);
            float sat = 1 - (x / (float)w);
            float val = x / (float)w;
            if (DEBUG) Log.d(TAG, String.format("hsv %s %s %s", hue, sat, val));
            mContent.setBackgroundColor(Color.HSVToColor(new float[]{hue, sat, val}));
            return true;
        }
