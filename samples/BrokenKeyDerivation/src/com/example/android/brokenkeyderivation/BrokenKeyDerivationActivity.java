...
public class BrokenKeyDerivationActivity extends Activity {
    ...
    private String retrieveData(String password) {
        ...
        if (isDataStoredWithInsecureKey()) {
            ...
            byte[] decryptedData = decryptData(retrieveEncryptedData(), insecureKey);
            SecretKey secureKey = deriveKeySecurely(password, keySize);
            storeDataEncryptedWithSecureKey(encryptData(decryptedData, secureKey));
            decryptedString = "Warning: data was encrypted with insecure key\n"
                    + new String(decryptedData, StandardCharsets.US_ASCII);
        } else {
            SecretKey secureKey = deriveKeySecurely(password, keySize);
            byte[] decryptedData = decryptData(retrieveEncryptedData(), secureKey);
    ...
    private static byte[] encryptOrDecrypt(byte[] data, SecretKey key, boolean isEncrypt) {
        try {
    ...
    private static byte[] decryptData(byte[] data, SecretKey key) {
    ...
    // Harcoded initial data.
    // encryptData("I hope it helped!", securelyDerivedKey("unguessable".getBytes(), 32));
    private byte[] encryptedData = new byte[] {
            33, 2, 38, 123, -80, 22, 68, 90, 47, -10, 81, 64, 97, -35, -59, 77, -86, -70, 95, 98,
            103, -49, 82, 115, 20, 17, 4, 50, -77, 14, -14, 104
    };
