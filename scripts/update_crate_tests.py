...
class Env(object):
    def __init__(self):
        try:
            ...
            sys.exit('ERROR: this script but be run from an Android tree.')
        self.cwd = os.getcwd()
    ...
    def query_rdep_tests(self, modules):
        ...
        for module in modules:
            for rdep in self.query_rdeps(module):
                ...
                if "rust_test_host" in rule_type:
                    rdep_tests[module.split(":")[1].split("--")[0]] = True
                elif "rust_test" in rule_type:
                    rdep_tests[module.split(":")[1].split("--")[0]] = False
        return rdep_tests
...
class TestMapping(object):
    ...
    def tests_to_mapping(self, tests):
        ...
        for test in tests:
            if tests[test] == True:
                test_mapping["presubmit"].append({"name": test, "host":True})
            else:
