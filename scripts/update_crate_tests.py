...
class TestMapping(object):
    ...
    def create_test_mapping(self, path):
        if self.env.setup == False:
            print("Failed to setup the environment.")
            return False
        if self.bazel.setup == False:
            print("Failed to setup bazel build.")
            return False
        tests = self.get_tests(path)
        if not bool(tests):
            print("No tests found - TEST_MAPPING not generated.")
            return True
...
def main():
    if len(sys.argv) == 2:
        ...
        path = None
    return TestMapping(path).create_test_mapping(None)
