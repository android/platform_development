...
def decide_license_type(cargo_license):
  ...
  for license_file in glob.glob("./LICENSE*"):
    file_name = license_file[2:]
    lowered_name = file_name.lower()
    if lowered_name == "license-apache":
      targets["APACHE2"] = file_name
    elif lowered_name == "license-mit":
      targets["MIT"] = file_name
  ...
  if not os.path.exists(license_file) and os.path.exists("LICENSE.txt"):
...
def parse_cargo_toml(cargo):
  ...
  with open(cargo, "r") as toml:
    for line in toml:
      ...
      if not cargo_license:
        match = LICENSE_MATCHER.match(line)
        if match:
          cargo_license = match.group(1)
