...
def display(indent, total, parent_total, node):
    ...
    percent = 0
    if total != 0:
      percent = 100 * node.size / float(total)
    parent_percent = 0
    if parent_total != 0:
      parent_percent = 100 * node.size / float(parent_total)
    print "%9d %6.2f%% %6.2f%% %8d %s%s %s %s %s" % (node.size, percent, parent_percent, node.number, indent, node.addr, fd.library, fd.function, fd.location)
