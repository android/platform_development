...
def GetNumFieldValid(native_heap):
  ...
  re_line = re.compile("z\s+(?P<zygote>\d+)\s+sz\s+(?P<size>\d+)\s+num\s+(?P<num_allocations>\d+)\s+bt\s+(?P<backtraces>.*)$")
  matched = 0
  for line in open(native_heap, "r"):
    ...
    if len(parts) > 7 and parts[0] == "z" and parts[2] == "sz":
      ...
      if m:
        ...
        backtraces = m.group('backtraces').split()
        if num_allocations == len(backtraces):
...
num_field_valid = GetNumFieldValid(native_heap)
if num_field_valid:
  print "The num allocations field is valid"
else:
  print "The num allocations field is not valid"
