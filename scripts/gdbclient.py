...
def generate_vscode_lldb_script(gdbpath, root, sysroot, binary_name, port, solib_search_path):
    ...
    res = {
        ...
        "processCreateCommands": ["gdb-remote localhost:{}".format(port)]
    }
...
def generate_setup_script(debugger_path, sysroot, linker_search_dir, binary_file, is64bit, port, debugger, connect_timeout=5):
    ...
    if debugger == "vscode-lldb":
        return generate_vscode_lldb_script(
            debugger_path, root, sysroot, binary_file.name, port, solib_search_path)
    elif debugger == "gdb":
