...
def ensure_linker(device, sysroot, interp):
    ...
    Returns a directory to add to the soinfo search path or None if no directory
    needs to be added.
    ...
    if os.path.exists(local_path):
        if posixpath.basename(interp) == posixpath.basename(interp_real):
            # Add the intepreter's directory to the search path.
            ...
            shutil.copy(local_path, os.path.join(result, os.path.basename(interp)))
            return result

    # Pull the system linker.
    result = make_temp_dir('gdbclient-linker-')
    device.pull(interp, os.path.join(result, os.path.basename(interp)))
    return result
