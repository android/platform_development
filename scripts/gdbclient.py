...
def generate_lldb_script(sysroot, binary_name, port, solib_search_path):
    commands = []
    commands.append('settings append target.exec-search-paths {}'.format(' '.join(solib_search_path)))

    commands.append('file ' + binary_name)
...
def generate_setup_script(debugger_path, sysroot, linker_search_dir, binary_file, is64bit, port, debugger, connect_timeout=5):
    ...
    if debugger == "vscode":
        ...
        return generate_lldb_script(sysroot, binary_file.name, port, solib_search_path)
    else:
    ...
    with binary_file:
        ...
        if tracer_pid == 0:
            ...
            if use_lldb:
                ...
                server_remote_path = "/data/local/tmp/{}-gdbserver".format(arch)
        ...
        if use_lldb:
            ...
            debugger_path = os.path.join(root, "prebuilts", "gdb", platform_name, "bin",
                                    "gdb")
        # Generate a gdb script.
        if use_lldb:
            debugger = 'lldb'
        elif args.setup_forwarding:
