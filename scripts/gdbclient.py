...
def main():
    ...
    if args.target_pid:
        ...
        for proc_name, pids in processes.iteritems():
            if pid in pids:
        ...
        binary = gdbrunner.pull_file(device, args.run_cmd[0], user=args.user)
    ...
    if not binary:
        try:
            binary = gdbrunner.pull_binary(device, pid=pid, user=args.user)
        except adb.ShellError:
            raise RuntimeError("failed to pull binary for PID {}".format(pid))

    arch = gdbrunner.get_binary_arch(binary)
