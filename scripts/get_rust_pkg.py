...
def cargo_show_find_pkgs(args, pkgs, found_pkgs):
  """Call cargo show to find direct dependent packages."""
  success = True
  all_build_deps = set()
  all_dev_deps = set()
  for pkg in pkgs:
...
def add_indirect_build_deps(args, found_pkgs, all_checked_pkgs):
  """Add all indirect dependencies and return a new found_pkgs."""
  all_pkg_deps = {}  # a map from P to its dependent tuple
  for (p, full_name, build_deps, dev_deps) in all_checked_pkgs:
  ...
  # TODO(chh): reverse_build_deps and reverse_dev_deps
  reverse_deps = {}  # a map from X to a set of P whose build_deps includes X
  reverse_dev_deps = {}  # a map from X to a set of P whose dev_deps includes X
  for p in all_pkgs:
    reverse_deps[p] = set()
    reverse_dev_deps[p] = set()
  for key in all_pkg_deps:
    ...
    for x in dev_deps:
      if x not in reverse_dev_deps:
        reverse_dev_deps[x] = set()
      reverse_dev_deps[x].add(p)
  # debug output
  echo(args, "Dump reverse_deps:")
  keys = sorted(reverse_deps.keys())
  for p in keys:
    echo(args, "  {}:{}".format(p, set2list(reverse_deps[p])))
  echo(args, "Dump reverse_dev_deps:")
  keys = sorted(reverse_dev_deps.keys())
  for p in keys:
    echo(args, "  {}:{}".format(p, set2list(reverse_dev_deps[p])))
  ...
  while remain_pkgs:
    ...
    for x in remain_pkgs:
      ...
      if x not in all_pkg_deps:
        continue
      _, _, x_deps, _ = all_pkg_deps[x]
      if x in reverse_deps and reverse_deps[x]:
        for p in reverse_deps[x]:
          ...
          all_pkg_deps[p] = (p, full_name, new_build_deps, dev_deps)
          if len(new_build_deps) > len(build_deps):
            next_remain_pkgs.add(p)
      if x in reverse_dev_deps and reverse_dev_deps[x]:
        for p in reverse_dev_deps[x]:
          if p not in all_pkg_deps or p not in all_pkg_deps:
            continue
          _, full_name, build_deps, dev_deps = all_pkg_deps[p]
          myself = set([p])
          new_dev_deps = dev_deps.union(x_deps).difference(myself)
          all_pkg_deps[p] = (p, full_name, build_deps, new_dev_deps)
...
def show_dependencies(args):
  ...
  found_pkgs = []
  success, build_deps, dev_deps = cargo_show_find_pkgs(args, pkgs, found_pkgs)
  if not found_pkgs:
    return False
  ...
  while build_pkgs or dev_pkgs:
    if build_pkgs:
      ...
      ok, build_pkgs, _ = cargo_show_find_pkgs(args, build_pkgs,
                                               other_build_pkgs)
    ...
    if dev_pkgs:
      ...
      ok, dev_pkgs, _ = cargo_show_find_pkgs(args, dev_pkgs, other_dev_pkgs)
      success = success and ok
  ...
  found_pkgs = add_indirect_build_deps(args, found_pkgs, all_pkgs)
