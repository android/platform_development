...
def compare_pkg_deps(pkg1, pkg2):
  ...
  base1, _, deps1, dev_deps1 = pkg1
  base2, _, deps2, dev_deps2 = pkg2
  if base1 in deps2 or base1 in dev_deps2:
    return -1  # pkg2 needs base1
  if base2 in deps1 or base2 in dev_deps1:
...
def check_dependencies(args, packages, dirs):
  ...
  for dest_dir in dirs:
    ...
    found_deps.append((base_name, pkg, deps, dev_deps))
  if not found_deps:
    return
  found_deps.sort(key=functools.cmp_to_key(compare_pkg_deps))
  max_pkg_length = 0
