...
def add_indirect_build_deps(args, found_pkgs, all_checked_pkgs):
  ...
  for key in all_pkg_deps:
    ...
    loop_pairs = [(build_deps, reverse_deps), (dev_deps, reverse_dev_deps)]
    for (deps, rev_deps) in loop_pairs:
      for x in deps:
        if x not in rev_deps:
          rev_deps[x] = set()
        rev_deps[x].add(p)
  # debug output
  for (name, deps) in [("", reverse_deps), ("dev ", reverse_dev_deps)]:
    echo(args, "Dump reverse_" + name + "deps:")
    keys = sorted(deps.keys())
    for p in keys:
      echo(args, "  {}:{}".format(p, set2list(deps[p])))
