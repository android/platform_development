...
def add_non_dev_dependencies(all_pkg_deps, visited, pkg):
  ...
  if pkg not in all_pkg_deps:
    return set()
  (_, build_deps, dev_deps) = all_pkg_deps[pkg]
  if pkg in visited:
    return build_deps
  visited.add(pkg)
  ...
  all_pkg_deps[pkg] = (pkg, build_deps, dev_deps)
  return build_deps
...
def add_indirect_build_deps(args, found_pkgs, other_build_pkgs, other_dev_pkgs):
  ...
  for (p, build_deps, dev_deps) in all_pkgs:
    all_pkg_deps[p] = (p, build_deps, dev_deps)
  visited = set()
