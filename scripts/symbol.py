...
def GetAbiFromToolchain(line, bits):
  if not line:
    return None

  toolchain_match = re.search("\/(aarch64|arm|mips|x86)\/", line)
  if toolchain_match:
  ...
  for line in lines:
    ...
    if trace_match:
      ...
      if len(trace_match.group(1)) == 16:
        ...
        ARCH = GetAbiFromToolchain(os.environ.get("ANDROID_TOOLCHAIN"), 64)
        if not ARCH:
          ARCH = "arm64"
      else:
        # 32 bit
        ...
        ARCH = GetAbiFromToolchain(os.environ.get("ANDROID_TOOLCHAIN_2ND_ARCH"), 32)
        if not ARCH:
          ARCH = GetAbiFromToolchain(os.environ.get("ANDROID_TOOLCHAIN"), 32)
          if not ARCH:
            ARCH = "arm"
      break
  if not ARCH:
