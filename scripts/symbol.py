...
def CallCppFilt(mangled_symbol):
  ...
  if not _CACHED_CXX_FILT:
    toolchains = None
    # If dependencies are declared properly in the build file, it should be on the $PATH already
    if shutil.which("llvm-cxxfilt") is not None:
      toolchains = ["llvm-cxxfilt"]
    if not toolchains:
