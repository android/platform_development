...
class SetArchTests(unittest.TestCase):
  ...
  def test_no_abi(self):
    ...
    # This is annoying. assertRaisesRegexp (Python2) warns in Python3 to use a different name.
    # But that name is not available in Python2. So we'll have to do this by hand.
    try:
      SetAbi([])
      self.assertTrue(False, "Expected exception")
    except Exception as e:
      self.assertEqual(str(e), "Could not determine arch from input, use --arch=XXX to specify it")
