...
class PipeCache:
  ...
  def GetToolPipe(self, cmd):
    ...
    while len(self._lru) >= _PIPE_MAX_OPEN:
      (old_cmd, open_pipe) = self._lru.pop(len(self._lru) - 1)
      del self._cmd2pipe[old_cmd]
      self.ClosePipe(open_pipe)

    pipe = self.OpenPipe(cmd)
    self._cmd2pipe[cmd_tuple] = pipe
    self._lru = [(cmd_tuple, pipe)] + self._lru
    return pipe

  def OpenPipe(self, cmd):
  ...
  def CloseAllPipes(self):
    for (old_cmd, open_pipe) in self._lru:
      self.ClosePipe(open_pipe)
    _cmd2pipe = {}
    _lru = []


_PIPE_ADDR2LINE_CACHE = PipeCache()
_PIPE_CPPFILT_CACHE = PipeCache()


# Pipe cleanup.

def CloseAllPipes():
  _PIPE_ADDR2LINE_CACHE.CloseAllPipes()
  _PIPE_CPPFILT_CACHE.CloseAllPipes()
...
def CallAddr2LineForSet(lib, unique_addrs):
  ...
  child = _PIPE_ADDR2LINE_CACHE.GetToolPipe(cmd)
...
def CallCppFilt(mangled_symbol):
  ...
  cmd = [ToolPath("c++filt")]
  proccess = _PIPE_CPPFILT_CACHE.GetToolPipe(cmd)
