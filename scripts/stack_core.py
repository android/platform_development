...
class TraceConverter:
  ...
  def __init__(self, lines):
    ...
    self.register_line = re.compile("(([ ]*[0-9a-z]{2} +[0-9a-f]" + width + "){4})")

    # Note that both trace and value line matching allow for variable amounts of
    # whitespace (e.g. \t). This is because the we want to allow for the self.STACK
    ...
    self.trace_line = re.compile("(.*)\#([0-9]+)[ \t]+(..)[ \t]+([0-9a-f]" + width + ")[ \t]+([^\r\n \t]*)( \((.*)\))?")  # pylint: disable-msg=C6310
    # Examples of matched value lines include:
    #   bea4170c  8018e4e9  /data/data/com.my.project/lib/libmyproject.so
    #   bea4170c  8018e4e9  /data/data/com.my.project/lib/libmyproject.so (symbol)
    #   03-25 00:51:05.530 I/DEBUG ( 65): bea4170c 8018e4e9 /data/data/com.my.project/lib/libmyproject.so
    # Again, note the spacing differences.
    self.value_line = re.compile("(.*)([0-9a-f]" + width + ")[ \t]+([0-9a-f]" + width + ")[ \t]+([^\r\n \t]*)( \((.*)\))?")
    ...
    self.code_line = re.compile("(.*)[ \t]*[a-f0-9]" + width +
                                "[ \t]*[a-f0-9]" + width +
                                "[ \t]*[a-f0-9]" + width +
                                "[ \t]*[a-f0-9]" + width +
                                "[ \t]*[a-f0-9]" + width +
  ...
  def CleanLines(self, lines):
    """Convert strings containing native crash to a self.STACK."""
  ...
  def ProcessLine(self, line):
    ...
    if self.trace_line.match(line):
      ...
      if area == self.UNKNOWN or area == self.HEAP or area == self.STACK:
        self.trace_lines.append((code_addr, "", area))
      else:
        # If a calls b which further calls c and c is inlined to b, we want to
        # display "a -> b -> c" in the self.STACK trace instead of just "a -> c"
    ...
    if self.value_line.match(line):
      ...
      if area == self.UNKNOWN or area == self.HEAP or area == self.STACK or not area:
