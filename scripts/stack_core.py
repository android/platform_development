...
class TraceConverter:
  ...
  zipinfo_name_offset_match = re.compile("^\s*(\S+)$\s*offset of local header from start of archive:\s*(\d+)", re.M)
  zipinfo_compressed_size_match = re.compile("^\s*compressed size:\s+(\d+)", re.M)
  trace_lines = []
  value_lines = []
  last_frame = -1
  width = "{8}"
  ...
  def GetInfo(self, central_info):
    match = self.zipinfo_name_offset_match.search(central_info)
    if not match:
      raise Exception("Cannot find zip name from zipinfo\n" + central_info)
    name = match.group(1)
    start = int(match.group(2))

    match = self.zipinfo_compressed_size_match.search(central_info)
    if not match:
      raise Exception("Cannot find compressed size from zipinfo\n" + central_info)

    return [name, start, start + int(match.group(1))]
  ...
  def GetLibFromApk(self, apk, offset):
    ...
    for line in cmd.stdout:
      ...
      if match:
        central_found = True
        if central_info:
          offset_entry = self.GetInfo(central_info)
          offset_list.append(offset_entry)
          if not file_entry and offset >= offset_entry[1] and offset < offset_entry[2]:
    ...
    if not file_entry:
      return None, None
    tmp_shared_lib = self.ExtractLibFromApk(apk_full_path, file_entry[0])
    if tmp_shared_lib:
      tmp_files[file_entry[0]] = tmp_shared_lib
      return file_entry[0], tmp_shared_lib
    return None, None
