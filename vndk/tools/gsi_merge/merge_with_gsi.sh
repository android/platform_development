...
usage() {
  echo
  echo "Merge GSI and partial build"
  echo
  echo "Create build artifact archive and image archive"
  echo "  by merging GSI and partial build of Pixel that was"
  echo "  built by using WITHOUT_SYSTEM_IMAGE=true"
  echo
  echo "Usage: merge_with_gsi.sh [args]"
  echo
  echo "  --h, --help"
  ...
  echo
  echo "Example:"
  echo "  merge_with_gsi.sh \\"
  echo "    --framework-dir cache/gsi \\"
...
MERGE_TARGET_FILES_OPTIONS+=" --framework-item-list=${ANDROID_BUILD_TOP}/development/vndk/tools/gsi_merge/framework_files.list"
MERGE_TARGET_FILES_OPTIONS+=" --framework-misc-info-keys=${ANDROID_BUILD_TOP}/development/vndk/tools/gsi_merge/misc_info_keys.list"
MERGE_TARGET_FILES_OPTIONS+=" --vendor-item-list=${ANDROID_BUILD_TOP}/development/vndk/tools/gsi_merge/partial_files.list"
