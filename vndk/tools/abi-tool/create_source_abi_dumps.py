...
def create_source_abi_reference_dump(soong_dir, is_vndk_lib_name, args):
    """Copy source based abi dumps in soong intermediates dir to
    ref dump dir """
    ...
    for base, dirnames, filenames in os.walk(soong_dir):
        for filename in filenames:
          ...
          if target_dir in os.path.dirname(path):
              ...
              if os.path.exists(source_dump_path):
                  arch = target_2nd_arch
          if arch is not None:
              copy_src_dump_to_dst(source_dump_path, arch, args.ref_dump_dir)
              num_copied += 1
    return num_copied

def copy_src_dump_to_dst(source_dump_path, arch, ref_dump_dir):
    """ gzip the source based abi-dump found and copy it to <ref-dump-dir>"""
    source_dump_dir = os.path.join(AOSP_DIR, ref_dump_dir, 'current', arch, \
                                   'source-based')
    makedirs(source_dump_dir, exist_ok=True)
    dst = os.path.join(source_dump_dir, os.path.basename(source_dump_path)) + \
        '.gz'
    with open(source_dump_path, 'rb') as dump_in:
      with gzip.open(dst, 'wb') as dump_out:
        shutil.copyfileobj(dump_in, dump_out)
