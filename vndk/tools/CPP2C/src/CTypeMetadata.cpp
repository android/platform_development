...
const std::string CTypeMetadata::getCType() const { return c_type_; }

const std::string CTypeMetadata::createCType() const {
  if (is_function_pointer_) {
    return function_pointer_name_left_ + function_pointer_name_right_;
  } else {
    if (!isEnum() && !isTemplate() && !isBuiltinType()) {
      return c_type_ + "*";
    } else {
      return c_type_;
    }
...
const std::string CTypeMetadata::createCTypeWithName(
...
bool CTypeMetadata::isTemplate() const { return is_template_; }

bool CTypeMetadata::isBuiltinType() const { return is_builtin_type_; }

std::string CTypeMetadata::getTemplateName() const { return template_name_; }

const std::vector<const CTypeMetadata> CTypeMetadata::getTemplateArgs() const {
  return template_args_;
}

CTypeMetadata::CTypeMetadata(
    ...
    const bool is_builtin_type, std::string template_name,
