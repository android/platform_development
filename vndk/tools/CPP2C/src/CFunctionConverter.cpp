...
CFunctionConverter::CFunctionConverter(const clang::CXXMethodDecl* c,
  ...
  cpp2c::addCustomClasses(customTypes_);
}
...
void CFunctionConverter::run(const clang::ASTContext* c, const Resources* cl) {
  ...
  for (unsigned int i = 0; i < method_decl_->getNumParams(); ++i) {
    ...
    auto customType = customTypes_.match(c_type_metadata);
    const bool is_custom_type = customType != nullptr;

    if (is_custom_type) {
      customType->addAsParameter(param_name, c_type_metadata, this,
                                 c_type_with_name);
    }
...
void CFunctionConverter::retrieveAsFunction(
  ...
  auto customType = customTypes_.match(c_return_type_metadata);
  const bool is_custom_type = customType != nullptr;

  if (is_custom_type) {
    customType->addAsReturnType(c_return_type_metadata, this);
  } else {
