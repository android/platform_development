...
TypeMetadata TypeConverter::determineCType(const clang::QualType& qt) {
  bool is_builtin_type = false;
  bool is_value_type = false;

  if (qt->isFunctionPointerType()) {
    handleFunctionPointer(qt);
  ...
  return TypeMetadata(
      c_type_, cast_type_, namespace_, c_definition_, qt->isEnumeralType(),
      is_pointer_, is_reference_, is_value_type, isConst(qt), is_void_type_,
      is_function_pointer_, is_template_, is_builtin_type, template_name_,
      template_args_, function_pointer_name_left_,
      function_pointer_name_right_);
}
