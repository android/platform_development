...
TypeMetadata TypeConverter::determineCType(const clang::QualType& type) {
  ...
  if (desugared_type->isFunctionPointerType()) {
    ...
    c_type_ = desugared_type.getAsString();
  ...
  return TypeMetadata(c_type_, cast_type_, namespace_, c_definition_,
                      desugared_type->isEnumeralType(), is_pointer_,
                      is_reference_, is_value_type, isConst(desugared_type),
                      is_void_type_, is_function_pointer_, is_template_,
                      is_builtin_type, template_name_, template_args_,
                      function_pointer_name_left_,
                      function_pointer_name_right_);
}
