...
static std::set<std::string> kImplementationsAdded;
...
void ClassMatchHandler::run(
  ...
  if (const clang::CXXMethodDecl* method_decl =
    ...
    for (const auto& it : implementations) {
      if (kImplementationsAdded.find(it.first) != kImplementationsAdded.end()) {
        continue;
      }

      kImplementationsAdded.insert(it.first);
      output_stream_ref.body_stream << it.second << "\n";
    }
