...
std::string MyFrontendAction::retrieve_preprocessor_define_name() {
  std::filesystem::path header_path = kHeaderForSrcFile;
  std::string fileName = header_path.filename().replace_extension().string();

  transform(begin(fileName), end(fileName), begin(fileName), ::toupper);
