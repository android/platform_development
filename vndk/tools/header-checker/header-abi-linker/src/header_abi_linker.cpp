...
class ExportedSymbolFilter {
 public:
  ExportedSymbolFilter(const SetLike *link_set,
                       std::set<std::string> *regex_matched_link_set,
                       const std::regex *vs_regex)
      : link_set_(link_set), regex_matched_link_set_(regex_matched_link_set),
        vs_regex_(vs_regex) {}

  bool operator()(const std::string &linker_set_key);

 private:
  const SetLike *link_set_;
  std::set<std::string> *regex_matched_link_set_;
  const std::regex *vs_regex_;
};

template<typename SetLike>
bool ExportedSymbolFilter<SetLike>::operator()(
    const std::string &linker_set_key) {
  // Filter out the symbol if it is not in link_set_ and doesn't match any
  // version script regex.
  return link_set_->find(linker_set_key) != link_set_->end() ||
         QueryRegexMatches(regex_matched_link_set_, vs_regex_, linker_set_key);
}
...
bool HeaderAbiLinker::LinkFunctions(
    ...
    abi_util::IRDumper *ir_dumper) {
  assert(reader != nullptr);
  ExportedSymbolFilter<decltype(function_decl_map_)> symbol_filter(
      &function_decl_map_,
      &functions_regex_matched_set,
      &functions_vs_regex_);
...
bool HeaderAbiLinker::LinkGlobalVars(
    ...
    abi_util::IRDumper *ir_dumper) {
  assert(reader != nullptr);
  ExportedSymbolFilter<decltype(globvar_decl_map_)> symbol_filter(
      &globvar_decl_map_,
