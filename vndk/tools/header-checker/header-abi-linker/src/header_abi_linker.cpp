...
class HeaderAbiLinker {
 ...
 private:
  ...
  void AddElfSymbols(abi_dump::TranslationUnit *linked_tu);
...
void HeaderAbiLinker::AddElfSymbols(abi_dump::TranslationUnit *linked_tu) {
  for (auto &&function : function_decl_set_) {
    linked_tu->add_elf_functions(function);
  }
 for (auto &&globvar : globvar_decl_set_) {
    linked_tu->add_elf_objects(globvar);
  }
}
