...
bool HeaderASTVisitor::VisitRecordDecl(const clang::RecordDecl *decl) {
  ...
  std::string source_file =
      record_decl_wrapper.GetDeclSourceFile(decl);
  if (source_file != current_file_name_)
    return true;
  //FIXME:Better Naming.
  std::unique_ptr<abi_dump::RecordDecl> record_decl =
...
bool HeaderASTVisitor::VisitFunctionDecl(const clang::FunctionDecl *decl) {
  FunctionDeclWrapper function_decl_wrapper(mangle_contextp_,
                                            ...
                                            decl);
  std::string source_file =
      function_decl_wrapper.GetDeclSourceFile(decl);
  if (source_file != current_file_name_)
    return true;
  std::unique_ptr<abi_dump::FunctionDecl> function_decl =
