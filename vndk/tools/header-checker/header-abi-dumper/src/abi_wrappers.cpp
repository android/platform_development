...
bool RecordDeclWrapper::SetupRecordVTable(
  ...
  std::map<uint64_t, clang::ThunkInfo> thunk_map(
      vtable_layout.vtable_thunk_begin(), vtable_layout.vtable_thunk_end());
  ...
  for (auto vtable_component : vtable_layout.vtable_components()) {
    ...
    if (thunk_map.find(index) != thunk_map.end()) {
      thunk_info = thunk_map[index];
    }
