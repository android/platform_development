...
bool RecordDeclWrapper::SetupRecordVTableComponent(
  ...
  abi_dump::VTableComponent::BasicVTableComponentAbi *basic_abi =
      added_vtable_component->mutable_basic_abi();
  if (!basic_abi) {
    return false;
  }
  basic_abi->set_kind(kind);
  basic_abi->set_value(value);
  basic_abi->set_mangled_component_name(mangled_component_name);
  basic_abi->set_linker_set_key(mangled_component_name  +
                                std::to_string(value) + std::to_string(kind));
  return true;
}
