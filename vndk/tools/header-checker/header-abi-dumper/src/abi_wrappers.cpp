...
std::string ABIWrapper::IsOrReferencesAnonRecordType(clang::QualType qual_type) {
  while (IsReferencingType(qual_type)) {
    qual_type = GetReferencedType(qual_type);
  }
  const clang::RecordDecl *anon_record = GetAnonymousRecord(qual_type);
  if (anon_record) {
    clang::SourceManager &sm = cip_->getSourceManager();
    clang::SourceLocation location = anon_record->getLocation();
    return " at " + location.printToString(sm);
  }
  return "";
}

std::string ABIWrapper::GetKeyForTypeId(clang::QualType qual_type) {
  const std::string &qual_type_name = QualTypeToString(qual_type);
  return qual_type_name +  IsOrReferencesAnonRecordType(qual_type);
}
...
bool ABIWrapper::CreateBasicNamedAndTypedDecl(
  ...
  std::string key_for_type_id = GetKeyForTypeId(canonical_type);
  typep->SetName(key_for_type_id);
  typep->SetLinkerSetKey(key_for_type_id);
