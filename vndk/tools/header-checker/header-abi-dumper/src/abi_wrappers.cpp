...
std::unique_ptr<abi_util::TypeIR> ABIWrapper::SetTypeKind(
  ...
  if (canonical_type.hasLocalQualifiers()) {
    auto qual_type_ir =
        std::unique_ptr<abi_util::QualifiedTypeIR>(
            new abi_util::QualifiedTypeIR());
  ...
  if (type_ptr->isPointerType()) {
    auto pointer_type_ir =
        std::unique_ptr<abi_util::TypeIR>(new abi_util::PointerTypeIR());
    pointer_type_ir->SetSourceFile(source_file);
    return pointer_type_ir;
  }
  if (type_ptr->isLValueReferenceType()) {
    auto lvalue_reference_type_ir = std::unique_ptr<abi_util::TypeIR>(
  ...
  if (type_ptr->isRValueReferenceType()) {
   auto rvalue_reference_type_ir = std::unique_ptr<abi_util::TypeIR>(
  ...
  if (type_ptr->isArrayType()) {
    auto array_type_ir =
        std::unique_ptr<abi_util::TypeIR>(new abi_util::ArrayTypeIR());
    array_type_ir->SetSourceFile(source_file);
    return array_type_ir;
  }
  if (type_ptr->isEnumeralType()) {
    auto enum_type_ir =
        std::unique_ptr<abi_util::TypeIR>(new abi_util::EnumTypeIR());
    return enum_type_ir;
  }
...
bool RecordDeclWrapper::SetupRecordInfo(abi_util::RecordTypeIR *record_declp,
  ...
  if (previous_record_stages_.empty()) {
  }
