...
bool ABIWrapper::CreateBasicNamedAndTypedDecl(
  ...
  if (!base_type->isVoidType() && type_and_status.is_valid_ && !typep) {
    llvm::errs() << "nullptr with valid type while creating basic type" << type_and_status.is_valid_ << "\n";
    return false;
  }
  return type_and_status.is_valid_ ? CreateBasicNamedAndTypedDecl(canonical_type, typep.get(),
                                      source_file)
...
TypeAndStatus ABIWrapper::SetTypeKind(
  ...
  if (canonical_type.hasLocalQualifiers()) {
    ...
    return TypeAndStatus(std::move(qual_type_ir));
  }
  ...
  if (type_ptr->isPointerType()) {
    ...
    return TypeAndStatus(std::move(pointer_type_ir));
  }
  if (type_ptr->isLValueReferenceType()) {
    ...
    return TypeAndStatus(std::move(lvalue_reference_type_ir));
  }
  if (type_ptr->isRValueReferenceType()) {
   ...
   return TypeAndStatus(std::move(rvalue_reference_type_ir));
  }
  if (type_ptr->isArrayType()) {
    auto array_type_ir = std::make_unique<abi_util::ArrayTypeIR>();
    array_type_ir->SetSourceFile(source_file);
    return TypeAndStatus(std::move(array_type_ir));
  }
  if (type_ptr->isEnumeralType()) {
    return TypeAndStatus(std::make_unique<abi_util::EnumTypeIR>());
  }
  if (type_ptr->isRecordType()) {
    assert(0);
    return TypeAndStatus(std::make_unique<abi_util::RecordTypeIR>());
  }
  if (type_ptr->isBuiltinType()) {
    ...
    return TypeAndStatus(std::move(builtin_type_ir));
  }
  if (type_ptr->isFunctionType()) {
      FunctionTypeWrapper function_type_wrapper(
          mangle_contextp_, ast_contextp_, cip_,
          static_cast<const clang::FunctionType *>(type_ptr), ir_dumper_,
  ...
  return TypeAndStatus(nullptr, false);
}
