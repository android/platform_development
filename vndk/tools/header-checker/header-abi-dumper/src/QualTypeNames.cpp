...
// external/clang/lib/Tooling/Core/QualTypeNames.cpp since there are
// some static methods needed by getFullyQualifiedTypeName not exported by the
// library (needed in order to change the printing policy to suppress the
// "struct" keyword while resolving types. This matters while mixing C / C++
// code.
