...
RecordFieldDiffInfo AbiDiffHelper::CompareRecordFields(
    const std::vector<abi_util::RecordFieldIR> &old_fields,
    const std::vector<abi_util::RecordFieldIR> &new_fields,
    std::deque<std::string> *type_queue,
    abi_util::DiffMessageIR::DiffKind diff_kind) {
  RecordFieldDiffInfo  diffed_removed_added_fields;
...
DiffStatus AbiDiffHelper::CompareRecordTypes(
  ...
  if (ir_diff_dumper_) {
    ...
    if ((record_type_diff_ir->IncompatibleDiffExists() ||
        record_type_diff_ir->IsExtended()) &&
