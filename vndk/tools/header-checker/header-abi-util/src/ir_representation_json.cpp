...
static const std::map<std::string, AccessSpecifierIR> access_json_to_ir(CreateReverseMap(access_ir_to_json));
...
static const std::map<std::string, RecordTypeIR::RecordKind> record_kind_json_to_ir(CreateReverseMap(record_kind_ir_to_json));
...
static const std::map<std::string, VTableComponentIR::Kind> vtable_component_kind_json_to_ir(CreateReverseMap(vtable_component_kind_ir_to_json));
...
static const std::map<std::string, ElfSymbolIR::ElfSymbolBinding> elf_symbol_binding_json_to_ir(CreateReverseMap(elf_symbol_binding_ir_to_json));
...
static inline AccessSpecifierIR AccessJsonToIR(const std::string &access) {
  return GetWithDefault(access_json_to_ir, access, AccessSpecifierIR::PublicAccess);
}
...
static inline RecordTypeIR::RecordKind RecordKindJsonToIR(const std::string &kind) {
  return GetWithDefault(record_kind_json_to_ir, kind, RecordTypeIR::struct_kind);
}
...
VTableComponentKindJsonToIR(const std::string &kind) {
  return GetWithDefault(vtable_component_kind_json_to_ir, kind, VTableComponentIR::Kind::UnusedFunctionPointer);
}

static inline std::string
ElfSymbolBindingIRToJson(ElfSymbolIR::ElfSymbolBinding binding) {
  return GetWithDefault(elf_symbol_binding_ir_to_json, binding, "global");
}

static inline ElfSymbolIR::ElfSymbolBinding
ElfSymbolBindingJsonToIR(const std::string &binding) {
  return GetWithDefault(elf_symbol_binding_json_to_ir, binding, ElfSymbolIR::ElfSymbolBinding::Global);
}
...
bool JsonToIRReader::ReadDump(const std::string &dump_file) {
  ...
  if (!reader.parse(input, tu_json, /* collectComments */ false)) {
    llvm::errs() << "Failed to parse JSON: " + reader.getFormattedErrorMessages() + "\n";
    return false;
  }
  bool ok = true;
...
std::vector<RecordFieldIR> JsonToIRReader::RecordFieldsJsonToIR(
  ...
  for (auto &&field : fields) {
    RecordFieldIR record_field_ir(
        ...
        field.GetUint("field_offset"), AccessJsonToIR(field.GetString("access")));
