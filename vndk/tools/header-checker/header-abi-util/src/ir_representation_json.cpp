...
void IRToJsonConverter::AddTemplateInfo(
  ...
  Json::Value &elements = type_decl["template_elements"];
  elements = JsonArray();
  for (auto &&template_element_ir : template_ir->GetTemplateElements()) {
    elements.append(template_element_ir.GetReferencedType());
  }
}
...
template <>
std::string JsonArrayRef<std::string>::Iterator::operator*() const {
...
void JsonToIRReader::ReadTemplateInfo(const JsonObjectRef &type_decl,
  ...
  for (auto &&referenced_type : type_decl.GetStrings("template_elements")) {
...
void JsonToIRReader::ReadRecordFields(
    const JsonObjectRef &record_type, RecordTypeIR *record_ir) {
...
void JsonToIRReader::ReadBaseSpecifiers(
    const JsonObjectRef &record_type, RecordTypeIR *record_ir) {
...
void JsonToIRReader::ReadVTableLayout(const JsonObjectRef &record_type,
  ...
  for (auto &&vtable_component :
       record_type.GetObjects("vtable_components")) {
