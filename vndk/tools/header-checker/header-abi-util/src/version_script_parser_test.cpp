...
TEST(VersionScriptParserTest, SmokeTest) {
  ...
  const std::map<std::string, ElfFunctionIR> &funcs = result->GetFunctions();
  ...
  const std::map<std::string, ElfObjectIR> &vars = result->GetVars();
...
TEST(VersionScriptParserTest, ParseExternCpp) {
  ...
  const std::set<std::string> &cpp_symbols = result->GetDemangledCppSymbols();
...
TEST(VersionScriptParserTest, ParseGlobPattern) {
  ...
  const std::set<std::string> &glob_patterns = result->GetGlobPatterns();
