...
static bool IntroducedLineSatisfiesArch(const std::string &line,
...
bool VersionScriptParser::SymbolInArchAndApiVersion(const std::string &line,
  ...
  // exported.
  if (!StringContains(line, "introduced") && StringContains(line, arch)) {
    return true;
  }
  if (line == "future") {
  ...
  if (std::regex_search(line, matcher1, match_clause1)) {
    ...
    IntroducedLineSatisfiesArch(line, arch)) {
