...
std::vector<std::string> ProtobufTextFormatToIRReader::ReadElfFunctions(
    const abi_dump::TranslationUnit &tu) {
  std::vector<std::string> elf_functions;
  for (auto &&elf_function : tu.elf_functions()) {
    elf_functions.emplace_back(elf_function);
  }
  return elf_functions;
}

std::vector<std::string> ProtobufTextFormatToIRReader::ReadElfObjects(
    const abi_dump::TranslationUnit &tu) {
  std::vector<std::string> elf_objects;
  for (auto &&elf_object : tu.elf_objects()) {
    elf_objects.emplace_back(elf_object);
  }
  return elf_objects;
}
...
bool IRDiffToProtobufConverter::AddTypeInfoDiff(
  ...
  old_type_info_protobuf->set_size(sizes.second);

  new_type_info_protobuf->set_alignment(alignments.first);
...
bool IRDiffToProtobufConverter::AddEnumUnderlyingTypeDiff(
  ...
  underlying_type_diff_protobuf->set_old_type(underlying_type_diff_ir->second);
  return true;
}
...
CompatibilityStatusIR ProtobufIRDiffDumper::GetCompatibilityStatusIR(
  ...
  // If -check-all-apis has been specified check unreferenced types as well.
  if (check_all_apis && (diff_tu_->unreferenced_enum_type_diffs().size() != 0 ||
  ...
  if (diff_tu_->enum_type_extension_diffs().size() != 0 ||
      ...
      diff_tu_->global_vars_added().size()) {
    return CompatibilityStatusIR::EXTENSION;
  }
