...
void TextFormatToIRReader::MergeGlobalVariables(
    const TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus>  *local_to_global_type_id_map) {
 for (auto &&global_variable_ir : addend.global_variables_) {
    MergeGlobalVariable(&global_variable_ir.second, addend,
                        local_to_global_type_id_map);
  }
}
