...
std::unique_ptr<IRDumper> IRDumper::CreateIRDumper(
  ...
  if (type == "protobuf") {
    return std::unique_ptr<IRDumper>(new ProtobufIRDumper(dump_path));
  }
...
std::unique_ptr<IRDiffDumper> IRDiffDumper::CreateIRDiffDumper(
    const std::string &type, const std::string &dump_path) {
  if (type == "protobuf") {
    return std::unique_ptr<IRDiffDumper>(new ProtobufIRDiffDumper(dump_path));
  }
  // Nothing else is supported yet.
  llvm::errs() << type << " message format is not supported yet!\n";
  return nullptr;
...
TextFormatToIRReader::CreateTextFormatToIRReader(
    const std::string &type, const std::string &dump_path) {
  if (type == "protobuf") {
    return std::unique_ptr<TextFormatToIRReader>(
        new ProtobufTextFormatToIRReader(dump_path));
  }
  // Nothing else is supported yet.
  llvm::errs() << type << " message format is not supported yet!\n";
  return nullptr;
