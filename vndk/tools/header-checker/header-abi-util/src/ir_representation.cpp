...
// TODO: Think about type hiddens very very carefully.
...
// TODO: s/types_list_/types_/g after you fix this in the linker and the
// differ. Comment out the Linker Merge Part.
// TODO: replace type-id with a constexpr

MergeStatus TextFormatToIRReader::IsBuiltinTypeNodePresent(
    const BuiltinTypeIR *builtin_type, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
  // TODO: Check the types map nistead because it'll be tougher maintaining
  // invariants. Add to the builtin_types vector ?
  ...
  // Remove this node from the addend graph, add an iterator to an std map :
  // TODO DONE: For now keep things simple and don't remove nodes from the
  // addend graph, just add to the old_type -> new translation cache instead.
  // Add to (type-id -> LinkableMessageIR) parent type_graph
  type_graph_.emplace(builtin_global_type_id, &((it.first)->second));
  //type_graph_.insert(std::make_pair(++max_type_id_, &(builtin_types_[builtin_linker_set_key])));
  // TODO:Add local type id to local_to_global_type_id_map.
...
MergeStatus TextFormatToIRReader::DoesUDTypeODRViolationExist(
    const TypeIR *ud_type, TextFormatToIRReader &addend,
    const std::string ud_type_unique_id_and_source,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map_) {
  // TODO: Add a static assert here that the type is a TagType.
  ...
  if (it == odr_list_map_.end()) {
    ...
    // name + source combination was seen in the parent graph.
    return MergeStatus(true, "type-hidden");
  }
  // TODO: Fix
  return MergeStatus(false, (*(it->second.begin()))->GetSelfType());
  // TODO: Change abi_diff_helpers to store type_cache with the decision cached
  // instead of just showing no diff. / or add a third value to the enum -> no 
  // decision / recursive diff.
  ...
  for (auto &contender_ud : it->second) {
    if (diff_helper.CompareAndDumpTypeDiff(contender_ud->GetSelfType(),
      ...
      return MergeStatus(true, contender_ud->GetSelfType());
    }
  }
  // TODO: Fix
  return MergeStatus(false, (*(it->second.begin()))->GetSelfType());
...
MergeStatus TextFormatToIRReader::IsTypeNodePresent(
    const TypeIR *addend_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
  std::string unique_type_id;
  switch(addend_node->GetKind()) {
    case RecordTypeKind:
      unique_type_id =
          static_cast<const RecordTypeIR *>(addend_node)->GetUniqueId();
      break;
    case EnumTypeKind:
      // TODO: Add Enum ODR Check -> that should be pretty efficient :P
      // TODO: One map is enough since enum and record names can't be the same,
      // tested it out.
      unique_type_id =
          static_cast<const EnumTypeIR *>(addend_node)->GetUniqueId();
      break;
    default:
  ...
  return DoesUDTypeODRViolationExist(
      addend_node, addend, unique_type_id + addend_node->GetSourceFile(),
...
// TODO: Collapse these 3 into one method.
void TextFormatToIRReader::MergeRecordFields(
    TextFormatToIRReader &addend, RecordTypeIR *added_node,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
...
MergeStatus TextFormatToIRReader::MergeRecordAndDependencies(
  const RecordTypeIR *addend_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
 ...
 //auto it = record_types_.emplace(added_type_id, *addend_node);
  ...
  return MergeStatus(true, added_type_id);
}

void TextFormatToIRReader::MergeEnumDependencies(
    TextFormatToIRReader &addend, EnumTypeIR *added_node,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
...
MergeStatus TextFormatToIRReader::MergeEnumType(
    const EnumTypeIR *addend_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
...
MergeStatus TextFormatToIRReader::MergeReferencingTypeInternalAndUpdateParent(
    TextFormatToIRReader &addend, const T *addend_node,
    ...
    // Create copy of addend_node
      ...
      if (it == parent_map->end()) {
        ...
        // TODO: Collapse these two into one since they should always be done in
        // lock step, also de-duplicate with code above.
        //
        // std::string self_global_type = "type-" + std::to_string(++max_type_id_);
        //TODO: Collapse this and the previous if condition into one.
        added_node.SetSelfType(updated_self_type_id);
        AddToMapAndTypeGraph(std::move(added_node), parent_map,
                             &type_graph_);
...
MergeStatus TextFormatToIRReader::MergeReferencingType(
    TextFormatToIRReader &addend, const TypeIR *addend_node,
    uint64_t rollback_id,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map,
  ...
  switch (addend_node->GetKind()) {
    case PointerTypeKind:
      return MergeReferencingTypeInternalAndUpdateParent(
          addend, static_cast<const PointerTypeIR *>(addend_node), rollback_id,
    ...
    case QualifiedTypeKind:
      return MergeReferencingTypeInternalAndUpdateParent(
          ...
          rollback_id, local_to_global_type_id_map, &qualified_types_,
          updated_self_type_id);
    case ArrayTypeKind:
      return MergeReferencingTypeInternalAndUpdateParent(
          addend, static_cast<const ArrayTypeIR *>(addend_node), rollback_id,
    ...
    case LvalueReferenceTypeKind:
      return MergeReferencingTypeInternalAndUpdateParent(
          ...
          rollback_id, local_to_global_type_id_map, &lvalue_reference_types_,
          updated_self_type_id);
    case RvalueReferenceTypeKind:
      return MergeReferencingTypeInternalAndUpdateParent(
          addend, static_cast<const RvalueReferenceTypeIR *>(addend_node),
          rollback_id, local_to_global_type_id_map, &rvalue_reference_types_,
          updated_self_type_id);
    default:
...
MergeStatus TextFormatToIRReader::MergeGenericReferringType(
  ...
  // descendants which are compound types (records), can refer to this type. If
  // it turns out that the 'referenced-type' of this type was not newly added to
  // the parent graph, roll back the max_type_id and take the 'added' (note: not
  // addend node), off the local_to_global_type_id map.
  uint64_t rollback_max_type_id = max_type_id_;
  // Add the record with type-id to the types_graph_, since if there are generic
  // reference types which refer to the record being added, they'll need to find
  // it's id in the map.
  std::string added_type_id = AllocateNewTypeId();
  ...
  local_to_global_type_id_map->emplace(addend_node->GetSelfType(),
                                       MergeStatus(true, added_type_id));
  // TODO: Set self type as well, not here becau'se we've not officially added a
  // node
  return MergeReferencingType(addend, addend_node, rollback_max_type_id,
                              local_to_global_type_id_map, added_type_id);
}
...
// TODO: This should be MergeNode instead, you can get any linkable / mergeable
// node from a LinkableMessageIR *
...
MergeStatus TextFormatToIRReader::MergeType(
    ...
    // parent graph. This does not add the node itself though. NO: IsNodePresent
    // also fixes all the references in the addend_node. After this, if its not
    // already present, we need to add it to the parent graph (preferably move
    // it), and update all the graphs / maps/ vectors maintained by the parent
    // graph.
    //
    // This tests for u.d types and builtin types whether the node was present
    // in the parent graph, since they are the only 'end points'.
    // First check the local_to_global_type_id_map;
...
void TextFormatToIRReader::MergeFunctionDeps(
    FunctionIR *added_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
...
void TextFormatToIRReader::MergeFunction(
    const FunctionIR *addend_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map) {
  const std::string &function_linkage_name = addend_node->GetLinkerSetKey();
  auto it = functions_.find(function_linkage_name);
...
void TextFormatToIRReader::MergeGraphs(TextFormatToIRReader &addend) {
  // Iterate through type_map_ of addend reader, on the way call MergeType on
  // each TypeIR *
  ...
  // TODO: Watch out for iterator invalidation if you're removing from the
  // addend type graph along the way.
