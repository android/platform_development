...
// TODO: Diffing could really do well with using unordered_maps instead of order
// ones, to find out common differences, use nother type: ordered map for
// functions and global variables. Faster for referred-type-id -> referring type
// lookup as well.
template <typename T>
...
class BasicTypeInfoIR {
 public:
  BasicTypeInfoIR(const std::string &name, const std::string &referenced_type,
                  const std::string &self_type,
                  const std::string linker_set_key, uint64_t size,
                  uint32_t alignment)
      : name_(name), referenced_type_(referenced_type), self_type_(self_type),
        linker_set_key_(linker_set_key), size_(size), alignment_(alignment) { }
...
class IRDumper {
 public:
  ...
  virtual void AddMaxTypeId(uint64_t max_type_id) = 0;
...
class TextFormatToIRReader {
 public:
  ...
  MergeStatus MergeReferencingTypeInternal(
      TextFormatToIRReader &addend, ReferencesOtherType *references_type,
      AbiElementMap<MergeStatus> *local_to_global_type_id_map);
  ...
  MergeStatus IsBuiltinTypeNodePresent(const BuiltinTypeIR *builtin_type,TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map);

  void MergeFunctionDeps(
    FunctionIR *addend_node, TextFormatToIRReader &addend,
    AbiElementMap<MergeStatus> *local_to_global_type_id_map);
  ...
  void MergeGraphs(TextFormatToIRReader &addend);
 ...
 protected:
  ...
  // TODO: Consider putting these back into vectors. Since now we don't need to 
  // 'de-duplicate' based on linker set key for types.
  // Should probably keep graph of records and builtin types by tag ids and
  // linker set keys respectively. tag ids to Records will help us with seeing
  // source files, builtin types, immediately figure out if its in the parent
  // graph.
  ...
  // TODO: Make this TypeIR *, so that abi-diffing between Records and enums
  // can be done easily. We can make a seperate map for Functions and GlobalVars
