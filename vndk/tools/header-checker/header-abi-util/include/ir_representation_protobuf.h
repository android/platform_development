...
inline abi_dump::AccessSpecifier AccessIRToProtobuf(AccessSpecifierIR access) {
  switch (access) {
    case AccessSpecifierIR::protected_access:
      return abi_dump::AccessSpecifier::protected_access;
    case AccessSpecifierIR::private_access:
...
inline AccessSpecifierIR AccessProtobufToIR(
  ...
  switch (access) {
    case abi_dump::AccessSpecifier::protected_access:
      return AccessSpecifierIR::protected_access;
    case abi_dump::AccessSpecifier::private_access:
      return AccessSpecifierIR::private_access;
    default:
      return AccessSpecifierIR::public_access;
  }
  return AccessSpecifierIR::public_access;
}
...
class ProtobufIRDiffDumper : public IRDiffDumper, public IRToProtobufConverter {
 public:
  ...
  virtual bool AddLinkableMessagesIR(const LinkableMessageIR *,
                                     ...
                                     const std::string &type_stack) override;

  virtual bool AddLinkableMessageIR(const LinkableMessageIR *) override;

  virtual void AddLibNameIR(const std::string &name) override;

  virtual void AddArchIR(const std::string &arch) override;

  virtual void AddCompatibilityStatusIR(CompatibilityStatusIR status) override;

  virtual bool Dump() override;

  virtual ~ProtobufIRDiffDumper() {}
 private:
  // User defined types.
  bool AddRecordTypeDiffIR(const RecordTypeIR *, const RecordTypeIR *,
                           const std::string &type_stack);

  bool AddEnumTypeDiffIR(const EnumTypeIR *, const EnumTypeIR *,
                         const std::string &type_stack);

  // Functions and global variables.
  bool AddFunctionDiffIR(const FunctionIR *, const FunctionIR *,
                         const std::string &type_stack);

  bool AddGlobalVarDiffIR(const GlobalVarIR *, const GlobalVarIR *,
                          const std::string &type_stack);

  bool AddLoneRecordTypeDiffIR(const RecordTypeIR *,
                               abi_dump::RecordType *abi_dump_record);

  bool AddLoneEnumTypeDiffIR(const EnumTypeIR *,
                             abi_dump::EnumType *abi_dump_enum);

  // Functions and global variables.
  bool AddLoneFunctionDiffIR(const FunctionIR *,
                             abi_dump::FunctionDecl *abi_dump_function);

  bool AddLoneGlobalVarDiffIR(const GlobalVarIR *,
                              abi_dump::GlobalVarDecl *abi_dump_globvar);
