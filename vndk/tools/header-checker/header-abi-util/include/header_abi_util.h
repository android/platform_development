...
std::vector<T> FindRemovedElements(
    const std::map<std::string, T> &old_elements_map,
    const std::map<std::string, T> &new_elements_map) {
  std::vector<T> removed_elements;
  for (auto &&map_element : old_elements_map) {
      auto element_key = map_element.first;
      auto new_element =
          new_elements_map.find(element_key);
...
inline void AddToMap(std::map<K,T> *dst,
                     Iterable &src,
                     F get_key) {
...
inline void AddToSet(std::set<K> *dst,
                     Iterable &src,
                     F get_key) {
  for (auto &&element : src) {
