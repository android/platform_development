...
def _are_sanitizer_variations(variations):
    ...
    # Return False if variations are empty.
    return any(_is_sanitizer_variation(v) for v in variations)


def _tag_to_variant_suffix(tag):
    """Maps a tag to variant suffix."""
    ...
    with open(lsdump_paths_file_path, 'r') as lsdump_paths_file:
        for line in lsdump_paths_file:
            ...
            for target in targets:
                ...
                if _are_sanitizer_variations(new_suffix):
                    continue
