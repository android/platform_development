...
def get_build_vars_for_product(names, product=None):
    ...
    if product is not None:
       cmd += 'command source build/envsetup.sh>/dev/null && lunch ' + product + '>/dev/null '\
                + ' && '
        #cmd += ['TARGET_PRODUCT=' + product]
    #cmd = ['make', '--no-print-directory', '-f', 'build/core/config.mk']
    cmd += 'command make --no-print-directory -f build/core/config.mk'
    environ = dict(os.environ)
    environ['CALLED_FROM_SETUP'] = 'true'
    environ['BUILD_SYSTEM'] = 'build/core'
    for build_var in names:
        build_cmd =  cmd + ' dumpvar-' + build_var
        proc = subprocess.Popen(build_cmd, stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE, env=environ,
                                cwd=AOSP_DIR, shell=True)
        out, err = proc.communicate()
        build_vars_list.append(out.decode('utf-8').strip())
