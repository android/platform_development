...
LSDUMP_PATHS_DIR = os.path.join(AOSP_DIR, 'out/lsdump_paths/')
...
def find_and_copy_lib_lsdumps(target, soong_dir, ref_dump_dir_stem,
    ...
    # reference  directory.
    cpu_variant_str = ''
    if target.cpu_variant != 'generic' and target.cpu_variant != '':
        cpu_variant_str = '-' + target.cpu_variant
...
def make_libs_for_all_arches_and_variants(libs, make_all_libs):
    if not make_all_libs:
        assert(libs)
...
def create_source_abi_reference_dumps(soong_dir, args, product,
    ...
    num_libs_copied = 0
    lsdump_paths_file = os.path.join(LSDUMP_PATHS_DIR, product)
    lsdump_paths_file = os.path.join(lsdump_paths_file, 'paths.txt')
...
def main():
    ...
    parser.add_argument('--no-make-lib', help='m -j lib.vendor while creating \
                        reference', default=False, action='store_true')
    parser.add_argument('--make-all-libs', help='make -j while creating \
                        reference', default=False, action='store_true')
    ...
    if (args.no_make_lib == False and args.libs) or args.make_all_libs == True:
        make_libs_for_all_arches_and_variants(args.libs, args.make_all_libs)
