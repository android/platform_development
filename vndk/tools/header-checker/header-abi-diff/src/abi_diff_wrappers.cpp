...
bool Diff<CXXBaseSpecifier>(const CXXBaseSpecifier &old_element,
  ...
  // Can be specialized for future changes in the format.
  if (DiffBasicTypeAbi(old_element.basic_abi().type_abi(),
                       new_element.basic_abi().type_abi()) ||
...
DiffWrapper<FunctionDecl, FunctionDeclDiff>::Get() {
  std::unique_ptr<FunctionDeclDiff> function_diff(new FunctionDeclDiff());
  if (DiffBasicTypeAbi(oldp_->basic_abi().type_abi(),
    ...
    assert(function_diff->mutable_return_type_diffs() != nullptr &&
           function_diff->mutable_return_type_diffs()->mutable_old() != nullptr &&
           function_diff->mutable_return_type_diffs()->mutable_new() != nullptr);
    *(function_diff->mutable_return_type_diffs()->mutable_old()) =
        oldp_->basic_abi();
    *(function_diff->mutable_return_type_diffs()->mutable_new_()) =
        newp_->basic_abi();
...
DiffWrapper<GlobalVarDecl, GlobalVarDeclDiff>::Get() {
  ...
  if (DiffBasicTypeAbi(oldp_->basic_abi().type_abi(),
    ...
    assert(global_var_diff->mutable_new() != nullptr);
