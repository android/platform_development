...
DiffWrapperBase::CompareCommonRecordFields(
    ...
    std::queue<std::string> *type_queue,
    abi_util::IRDiffDumper::DiffKind diff_kind) {
  // It is implied that the offsets are equal.
  if (
...
DiffWrapperBase::CompareRecordFields(
  ...
  std::map<uint64_t, const abi_util::RecordFieldIR *> old_fields_map;
  std::map<uint64_t, const abi_util::RecordFieldIR *> new_fields_map;
  // Map fields on the basis of their in memory offset and not name. This allows
  // us to avoid reports of breakages on changing the name of a field.
  abi_util::AddToMap(&old_fields_map, old_fields,
...
void DiffWrapperBase::CompareTemplateInfo(
    ...
    std::queue<std::string> *type_queue,
    abi_util::IRDiffDumper::DiffKind diff_kind) {
  assert(old_template_elements.size() == new_template_elements.size());
  int i = 0;
  while (i < old_template_elements.size()) {
    const abi_util::TemplateElementIR &old_template_element =
        old_template_elements.at(i);
    const abi_util::TemplateElementIR &new_template_element =
        new_template_elements.at(i);
...
DiffStatus DiffWrapperBase::CompareBuiltinTypes(
  ...
  // TODO: we should also consider the fundamental type of built-in type like
  // float vs integral type vs char.
...
DiffStatus DiffWrapperBase::CompareAndDumpTypeDiff(
  ...
  if (old_kind != new_kind) {
    return CompareDistinctKindMessages(old_it->second, new_it->second);
  }
