...
class HeaderAbiDiff {
 public:
  ...
  HeaderAbiDiff(const std::string &old_dump, const std::string &new_dump,
      ...
      : old_dump_(old_dump), new_dump_(new_dump), cr_(compatibility_report),
        ignore_symbols_(ignore_symbols) { }
 ...
 private:
  ...
  template <typename T, typename TDiff>
  bool static DumpDiffElements(
      google::protobuf::RepeatedPtrField<TDiff> *dst,
      std::vector<std::pair<const T *, const T *>> &pairs,
      const std::set<std::string> &ignore_symbols);

  template <typename T>
  bool static DumpLoneElements(google::protobuf::RepeatedPtrField<T> *dst,
 ...
 private:
  ...
  const std::set<std::string> &ignore_symbols_;
};
