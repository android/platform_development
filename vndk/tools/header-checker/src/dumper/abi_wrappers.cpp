...
// A mangled anonymous enum name ends with $_<number> or Ut<number>_ where the number may be inconsistent between translation units.
// This function replaces the name with $ followed by the lexicographically smallest field name.
static std::string GetAnonymousEnumUniqueId(llvm::StringRef mangled_name, const clang::EnumDecl *enum_decl) {
  ...
  if (std::regex_search(mangled_name_str, match_result, std::regex(R"((\\$_\\d+)(E?)$)"))) {
    ...
    if (!mangled_name.endswith(old_suffix)) {
      llvm::errs() << "Unexpected length of anonymous enum type name: " << mangled_name << "\n";
      ::exit(1);
    }
  } else if (std::regex_search(mangled_name_str, match_result, std::regex(R"(Ut\\d*_(E?)$)"))) {
  ...
  std::string new_suffix = std::to_string(smallest_enum_name.length()) + smallest_enum_name + nested_name_suffix;
