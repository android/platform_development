...
static llvm::cl::opt<std::size_t> sources_per_thread(
    "sources-per-thread", llvm::cl::desc("Specify number of input dump files each thread parses, for debugging merging type graphs"),
    llvm::cl::init(7),
    llvm::cl::Hidden);
...
HeaderAbiLinker::ReadInputDumpFiles() {
  ...
  std::size_t num_threads = sources_per_thread < dump_files_.size() ?
      std::min(dump_files_.size() / sources_per_thread, max_threads) : 0;
