...
RootDirs ParseRootDirs(const std::vector<std::string> &args) {
  ...
  for (const std::string_view arg : args) {
    ...
    llvm::SmallString<256> norm_replacement(replacement.begin(), replacement.end());
    llvm::sys::path::remove_dots(norm_replacement, /* remove_dot_dot = */ true);
    root_dirs.emplace_back(NormalizePath(path, no_root_dirs), std::string(norm_replacement));
  }
  if (root_dirs.empty()) {
  ...
  // matching root dir.
  std::sort(root_dirs.begin(), root_dirs.end(), [](RootDir &first, RootDir &second){
    return first.path_.size() > second.path_.size();
  });
  return root_dirs;
}
...
std::string NormalizePath(std::string_view path,
  ...
  for (const RootDir &root_dir: root_dirs) {
    ...
    // not use it.
    if (!norm_path.startswith(root_dir.path_)) {
      continue;
    }
    if (norm_path.size() == root_dir.path_.size()) {
      return root_dir.replacement_;
    }
    llvm::StringRef suffix = norm_path.substr(root_dir.path_.size());
    if (suffix.startswith(separator)) {
      if (root_dir.replacement_.empty()) {
        return suffix.substr(separator.size()).str();
      }
      // replacement == "/"
      if (llvm::StringRef(root_dir.replacement_).endswith(separator)) {
        return root_dir.replacement_ + suffix.substr(separator.size()).str();
      }
      return root_dir.replacement_ + suffix.str();
    }
  }
