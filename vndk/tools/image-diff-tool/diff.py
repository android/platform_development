...
def make_filter_from_whitelists(whitelists, targets):
  ...
  def diff_with_ignore_lines(filename, targets, prefixes):
    ...
    return (len(file_digest_respect_ignore) == len(targets) and
  ...
  def whitelist_filter(filename):
    ...
    if filename.lower() in whitelist_line:
      ...
      return diff_with_ignore_lines(filename, targets, ignored_prefixes)
    return False
