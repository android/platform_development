...
def make_filter_from_whitelists(whitelists, all_targets):
  ...
  """
  ignored_suffix = set()
  ignored_directory = set()
  ignored_pathname = set()
  ignored_line = defaultdict(list)
    ...
    with open(whitelist, 'rb') as f:
      for line in f:
        ...
        if '=' in pat:
          ...
          if prefixes:
            ignored_line[filename.lower()].extend(prefixes)
        else:
          pat = pat.lower()
          if pat.startswith('*.'):
            ignored_suffix.add(pat[1:])
          elif pat.endswith('/'):
            ignored_directory.add(pat)
          elif pat:
            ignored_pathname.add(pat)
  ...
  def whitelist_filter(filename):
    ...
    if any(norm_filename.endswith(suffix) for suffix in ignored_suffix):
      return True
    if any(norm_filename.startswith(prefix) for prefix in ignored_directory):
      return True
    if norm_filename in ignored_pathname:
      return True
    if norm_filename in ignored_line:
      ignored_prefixes = ignored_line[norm_filename]
