...
function task_html(text) {
  return '<li><span class="display" id="dep' + counter + '">' +
  text + '</span>' + '<input type="text" class="edit" style="display:none"/>' +
  '<input type="submit" class="delete" value="X">' +'</li>';
}

// ccounter is a global var
function code_html(text) {
  return '<li><span id="code' + ccounter + '">'+ text +
  '</span><input type="submit" class="delete" value="X">' +'</li>';
}

function item_html(done, text) {
  if (done) {
    return '<pre><a class="list-group-item list-group-item-success"' +
           ' onclick="set_item(this)">' + text + '</a></pre>';
  }
  else {
    return '<pre><a class="list-group-item list-group-item-danger"' +
           ' onclick="set_item(this)">' + text + '</a></pre>';
  }
}
...
function enter_task () {
  ...
  $('#deps_list').append( task_html(text) );
...
function enter_code () {
  ...
  $('#code_list').append( code_html(text) );
  $('.delete').click(function(){ $(this).parent().remove(); });
  ccounter++;
};
...
function on_load() {
  $.getJSON('/get_started', {
    ...
    const len = done.length;
    for (let i = 0; i < len; i++) {
      $('#item_list').append( item_html(done[i], lst[i]) );
    }
    const pattern = data.pattern;
    const is_regex = data.is_regex;
    $('#add_pattern').children().first().val(data.pattern);
    $('#add_pattern').children().eq(1).attr('checked', data.is_regex);
  });
}
