...
class ELFLinker(object):
    ...
    def compute_vndk(self, sp_hals, vndk_stable, vndk_customized_for_system,
        ...
        vndk_ext_pattern = re.compile('^/(?:(?:system)|(?:vendor))/'
                                      'lib(?:64)?/vndk-(?:\\d+-)?ext/')

        vndk_fwk_ver = 26  # FIXME: This should be user-specified argument.
        vndk_vnd_ver = 26  # FIXME: This should be user-specified argument.

        def is_vndk_ext_path(path):
            return vndk_ext_pattern.match(path)

        def get_vndk_core_path(lib):
            lib_dir = 'lib' if lib.elf.is_32bit else 'lib64'
            lib_name = os.path.basename(lib.path)
            return os.path.join('/', 'system', lib_dir, 'vndk-' + vndk_fwk_ver,
                                lib_name)
        ...
        if not generic_refs:
            ...
            while vndk_candidates:
                ...
                # Each VNDK libraries should have their ELFLinkData instance.
                ...
                assert not (set(lib.path for lib in vndk_fwk_ext) -
                            set(lib.path for lib in vndk_core))
                assert not (set(lib.path for lib in vndk_vnd_ext) -
                            set(lib.path for lib in vndk_core))
                ...
                def add_to_vndk_core(lib):
                    """Add the library to vndk core."""
                    path = lib.path
                    if is_vndk_ext_path(lib.path):
                        path = get_vndk_core_path(lib)
                        assert path != lib.path
