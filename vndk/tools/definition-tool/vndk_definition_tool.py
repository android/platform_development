...
class CheckDepGraphCommand(ELFGraphCommand):
    def __init__(self):
        super(CheckDepGraphCommand, self).__init__(
                'dep-graph', help='Show the eligible dependencies graph')

    def add_argparser_options(self, parser):
        super(CheckDepGraphCommand, self).add_argparser_options(parser)
    ...
    def _get_tag_from_lib(self, lib, tags):
        ...
        TAG_DEP_TABLE = {'vendor': ('ll_ndk', 'sp_ndk', 'vndk_sp', \
                                  'vndk', 'vndk_indirect')}
        ...
        for tag in TAGGED_LIB_DICT_FIELDS:
            if tag == 'vendor':
                ...
                tag_hierarchy.append('system.%s.%s' % \
                    ('public' if tag in eligible_for_vendor else 'private', \
                     tag))
        lib_name = os.path.basename(lib.path)
...
def main():
    ...
    register_subcmd(CheckDepGraphCommand())
