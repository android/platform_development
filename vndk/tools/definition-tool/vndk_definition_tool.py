...
class ListBuildSplitBlockerCommand(ELFGraphCommand):
    def __init__(self):
        super(ListBuildSplitBlockerCommand, self).__init__(
                'list-build-split-blocker',
                help='List build split blockers')

    def add_argparser_options(self, parser):
        super(ListBuildSplitBlockerCommand, self).add_argparser_options(parser)
    ...
    def main(self, args):
        ...
        results = []
        for name, lib in graph.lib_pt[PT_SYSTEM].items():
            if any(dep.path.startswith('/vendor') for dep in lib.deps):
                results.append((name, sorted_lib_path_list(lib.deps)))
        results.sort()
...
def main():
    ...
    register_subcmd(ListBuildSplitBlockerCommand())
