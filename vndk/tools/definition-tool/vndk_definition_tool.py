...
class VNDKLibDir(defaultnamedtuple('VNDKLibDir', 'lib32 lib64', {})):
    """VNDKLibDir is a dict which maps version to VNDK-SP and VNDK directory
    paths."""


    def get_versions(self, elf_class):
    ...
    def add_version(self, version):
        for lib_dir, elf_class in (('lib', ELF.ELFCLASS32),
            ...
            vndk_sp_name = self.create_vndk_sp_dir_name(version)
            vndk_name = self.create_vndk_dir_name(version)

            versions = self.get_versions(elf_class)
            versions[version] = VNDKLibTuple(
    ...
    def create_from_version(cls, version):
        ...
        vndk_lib_dirs = VNDKLibDir()
        vndk_lib_dirs.add_version(version)
        return vndk_lib_dirs
    ...
    def get_matched_version(self, vendor_dirs):
        ro_vndk_version = self.get_ro_vndk_version(vendor_dirs)
        if ro_vndk_version is not None:
            return ro_vndk_version

        versions = self.sorted_version(self.get_versions(ELF.ELFCLASS32).keys())
        if not versions:
            return 'current'
        return versions[0]
...
class ELFLinker(object):
    ...
    def _is_in_vndk_dir(cls, path):
        component = VNDKLibDir.extract_path_component(path, 3)
        if not component:
            return False
    ...
    def _resolve_elf_class_deps(self, lib_dir, elf_class, generic_refs):
        ...
        system_vndk_sp_libs, system_vndk_libs, system_libs = \
                self.vndk_lib_dirs.classify_vndk_libs(system_lib_dict.values())

        vendor_lib_dict = self.lib_pt[PT_VENDOR].get_lib_dict(elf_class)
        vendor_vndk_sp_libs, vendor_vndk_libs, vendor_libs = \
                self.vndk_lib_dirs.classify_vndk_libs(vendor_lib_dict.values())
        ...
        vndk_lib_dirs = self.vndk_lib_dirs.get_versions(elf_class)
        for version, (vndk_sp_dirs, vndk_dirs) in vndk_lib_dirs.items():
        ...
        # Resolve vendor libs.
        vndk_sp_dirs, vndk_dirs = vndk_lib_dirs[self.ro_vndk_version]
    ...
    def _create_internal(scan_elf_files, system_dirs, system_dirs_as_vendor,
        ...
        ro_vndk_version = vndk_lib_dirs.get_matched_version(vendor_dirs)
