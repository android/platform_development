        ...
        if self.is_32bit:
            ...
            def parse_elf_phdr(offset):
                try:
                    ...
                    return Elf_Phdr(p[0], p[6], p[1], p[2], p[3], p[4], p[5],
                                    p[7])
                except struct.error:
...
class VNDKCommand(VNDKCommandBase):
    ...
    def _print_file_size_output(self, graph, vndk_lib, file=sys.stderr):
        ...
        SEPARATOR = ('----------', None, None, None, None, None)
        ...
        for tag in _VNDK_RESULT_FIELD_NAMES:
            ...
            writer.writerow(['Subtotal' + tag + ' (32-bit)'] + total)
        ...
        for tag in _VNDK_RESULT_FIELD_NAMES:
            ...
            total = calc_total_size(lib_set)
            writer.writerow(['Subtotal ' + tag + ' (64-bit)'] + total)

        writer.writerow(SEPARATOR)
        for tag in _VNDK_RESULT_FIELD_NAMES:
            total = calc_total_size(getattr(vndk_lib, tag))
            writer.writerow(['Subtotal ' + tag + ' (both)'] + total)

        # Calculate the summation of all ELF files.
        writer.writerow(SEPARATOR)
        writer.writerow(['Total'] + calc_total_size(graph.all_libs()))
