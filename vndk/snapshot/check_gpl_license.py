    ...
    def check_gpl_projects(self):
        ...
        for notice_file_path in notice_files:
            with open(notice_file_path, 'r') as notice_file:
                if pattern in notice_file.read():
                    gpl_projects.append(
                        os.path.splitext(os.path.basename(notice_file_path))[0])

        if len(gpl_projects) == 0:
        ...
        for lib in gpl_projects:
            if lib in module_paths:
                ...
                if not revision:
                    raise RuntimeError(
                        'No project found for {path} in {manifest}'.
                        format(path=module_path, manifest=self.MANIFEST_XML))
