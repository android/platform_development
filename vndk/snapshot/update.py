...
def fetch_artifact(branch, build, pattern, custom_name):
    ...
    # Sets the downloaded artifact's file name to 'custom_name' post-fetch
    if custom_name:
...
def install_snapshot(branch, build):
    ...
    if branch and build:
        ...
        fetch_artifact(branch, build, artifact_pattern, None)
        ...
        fetch_artifact(branch, build, manifest_file, 'manifest.xml')

        # remove fetch_artifact data file
        fetch_artifact_datafile_pattern = '.fetch_artifact*.dat'
        fetch_artifact_datafiles = glob.glob(fetch_artifact_datafile_pattern)
        if len(fetch_artifact_datafiles) == 0:
            logger().warning('No such file: {}'.format(
                fetch_artifact_datafile_pattern))
        else:
            for file in fetch_artifact_datafiles:
                os.unlink(file)
    else:
    ...
    try:
        ...
        if artifact_cnt < 4:
            raise RuntimeError(
                'Number of artifacts found in {path} is {cnt}. There should '
                'be four.'.format(path=artifact_dir, cnt=artifact_cnt))
