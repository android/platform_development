...
class GenBuildFile(object):
    ...
    def _get_etc_paths(self):
        ...
        for etc_module in self.ETC_MODULES:
            etc_name, _ = os.path.splitext(etc_module)
            etc_path = glob.glob(
                '{}/*/*/{}*'.format(self._install_dir, etc_name))[0]
    ...
    def _gen_bp_phony(self, variant):
        ...
        """
        required = []
        for prebuilts in (self._vndk_core[variant],
                          self._vndk_sp[variant]):
    ...
    def _gen_vndk_shared_prebuilts(self, prebuilts, variant, is_vndk_sp):
        ...
        for prebuilt in prebuilts:
            build_rules.append(
                self._gen_vndk_shared_prebuilt(prebuilt, variant,
                                               is_vndk_sp))
        return build_rules
