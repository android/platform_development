...
class GenBuildFile(object):
    ...
    def __init__(self, install_dir, vndk_version):
        ...
        self._etc_suffix = utils.get_etc_suffix(install_dir)
        self._variants = utils.get_snapshot_variants(install_dir)
    ...
    def _get_versioned_name(self, prebuilt, variant_arch, is_etc):
        ...
        For a VNDK shared lib: "libfoo.so"
                            -> "libfoo.vndk.{version}.{variant_arch}.vendor"
        For an ETC text file: "foo.txt" -> "foo.{version}.txt"

        Args:
          prebuilt: string, name of the prebuilt object
          variant_arch: string, arch of VNDK snapshot variant (e.g. 'arm64')
        ...
        if is_etc:
            ...
            versioned_name = '{}.vndk.{}.{}.vendor'.format(
                name, self._vndk_version, variant_arch)
    ...
    def _gen_etc_prebuilt(self, prebuilt):
        ...
        """
        name, _ = os.path.splitext(prebuilt)
        etc_path = utils.find(self._install_dir, [name+self._etc_suffix])[0]
        ...
        return (
            ...
            'LOCAL_SRC_FILES := aosp_$(TARGET_ARCH)_ab/'
            '{etc_sub_path}\n'
    ...
    def _gen_bp_phony(self, variant):
        """Generates build rule for phony package 'vndk_v{ver}_{variant_arch}'.

        Args:
          variant: string, VNDK snapshot variant (e.g. 'aosp_arm64_ab')
        """
        required = []
        variant_arch = utils.arch_from_variant(variant)
        for prebuilts in (self._vndk_core[variant], self._vndk_sp[variant]):
            for prebuilt in prebuilts:
                required.append(
                    self._get_versioned_name(prebuilt, variant_arch, False))
        ...
        return ('phony {{\n'
                '{ind}name: "vndk_v{ver}_{variant_arch}",\n'
                '{required_buildrule}'
                '}}\n'.format(
                    ind=self.INDENT,
    ...
    def _gen_vndk_shared_prebuilt(self, prebuilt, variant, is_vndk_sp):
        ...
        vendor_available = 'false' if prebuilt in self._vndk_private[variant] else 'true'
        if is_vndk_sp:
