...
class GenBuildFile(object):
    ...
    def generate_android_bp(self):
        ...
        def gen_for_variant(arch, is_binder32=False):
            ...
            if variant_include_path != include_path:
    ...
    def _gen_vndk_shared_prebuilt(self,
        ...
        def get_arch_props(prebuilt, arch):
            ...
            def list_to_prop_value(l):
                return ",".join(['"%s"' % d for d in l])

            for src in sorted(src_paths):
                prop_path = os.path.join(src_root, src+'.prop')
                ...
                if 'ExportedDirs' in props:
                    ...
                    include_dirs = ('{ind}{ind}{ind}'
                        'export_include_dirs: [{dirs}],\n'.format(
                            ind=self.INDENT,
                            dirs=list_to_prop_value(l)))
                system_include_dirs = ''
                if 'ExportedSystemDirs' in props:
                    l = ['include/%s' % d for d in props['ExportedSystemDirs']]
                    system_include_dirs = ('{ind}{ind}{ind}'
                        'export_system_include_dirs: [{dirs}],\n'.format(
                            ind=self.INDENT,
                            dirs=list_to_prop_value(l)))
                flags = ''
                if 'ExportedFlags' in props:
                    l = props['ExportedFlags']
                    flags = ('{ind}{ind}{ind}export_flags: [{flags}],\n'.format(
                        ind=self.INDENT,
                        flags=list_to_prop_value(l)))
                relative_install_path = ''
