...
class GenBuildFile(object):
    ...
    def _gen_bp_phony(self, target_arch):
        ...
        """
        required = []
        for prebuilts in (self._vndk_core[target_arch],
                          self._vndk_sp[target_arch]):
    ...
    def _gen_vndk_shared_prebuilts(self, prebuilts, target_arch, is_vndk_sp):
        ...
        for prebuilt in prebuilts:
            build_rules.append(
                self._gen_vndk_shared_prebuilt(prebuilt, target_arch,
                                               is_vndk_sp))
        return build_rules
    ...
    def _gen_vndk_shared_prebuilt(self, prebuilt, target_arch, is_vndk_sp):
        ...
        def get_notice_file(prebuilt):
            ...
            if len(notice_files) > 0:
                notice = '{ind}notice: "{notice_file_path}",\n'.format(
                    ind=self.INDENT,
                    notice_file_path=os.path.join('../NOTICE_FILES',
                                                  notice_files[0]))
            return notice
        ...
        vendor_available = str(
            prebuilt not in self._vndk_private[target_arch]).lower()
        if is_vndk_sp:
