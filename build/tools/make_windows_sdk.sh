...
function package() {
    ...
    # Copy the SDK Updater (aka sdklauncher) to the root of the SDK (it was copied in tools above)
    # and move it also in SDK/tools/lib (so that tools updates can update the root one too)
    cp "$TOOLS/sdklauncher.exe" "$TEMP_SDK_DIR/SDK Updater.exe"
    mv "$TOOLS/sdklauncher.exe" "$LIB/SDK Updater.exe"
