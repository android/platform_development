...
function package() {
    ...
    # Copy the SDK Launcher (aka sdklauncher) to the root of the SDK (it was copied in tools above)
    # and move it also in SDK/tools/lib (so that tools updates can update the root one too)
    cp "$TOOLS/sdklauncher.exe" "$TEMP_SDK_DIR/SDK Launcher.exe"
    mv "$TOOLS/sdklauncher.exe" "$LIB/SDK Launcher.exe"
