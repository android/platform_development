...
def strip_space(from_str):
    """Strip out spaces from string from_str."""
...
class Function(object):
    ...
    def __init__(self, tokens, file_name, module):
        ...
        # callee will be used in dlsym and may be identical to others
        # for functions with variable argument lists.
        self.callee = self.name
        if self.name in VA_FUNCTIONS:
            self.callee = VA_FUNCTIONS[self.name][0]
        self.file_name = short_header_path(file_name)
        self.module = module
...
class Header(object):
    ...
    def build_functions(self, processed_functions):
        ...
        for token in self.cursor.get_tokens():
            # TODO(danalbert): Check for @stable rather than U_STABLE.
            # U_STABLE is not applied everywhere it ought to be, and may go
            # away in the future.
