...
assert TOP is not None, 'ANDROID_BUILD_TOP not set.\n'
...
clang.cindex.Config.set_library_path(os.path.join(TOP,
...
def parse_parameters(tokens, name):
  ...
  return (', '.join(param_types), ', '.join(param_vars),
          param_list, has_va, last_param, errorcode_var)
...
def is_valid_function(tokens):
  """Check if the givens tokens look like a function."""
...
"""
  ...
  def render(self, functions, includes, modules):
    ...
    for f in functions:
      ...
      if f.errorcode_var is not None:
        print '  if (syms[%d] == NULL) {' % (idx,)
        print '    *%s = U_UNSUPPORTED_ERROR;' % (f.errorcode_var,)
        if f.return_void:
          print '    return;'
        else:
          print '    return (%s)0;' % (f.result_type,)
        print '  }'

      print '  ptr = (%s(*)(%s))syms[%d];' % (f.result_type, f.param_types,
                                              idx)
...
# Presumably icu4c is under 'external/icu/icu4c/source'.
base_path = os.path.join(TOP, 'external/icu/icu4c/source')
...
if __name__ == '__main__':
  ...
  tmpl = Template()
  tmpl.render(functions=collection, includes=includes, modules=modules)
