...
uint32_t JNIHelper::LoadTexture( const char* file_name )
{
    ...
    mid = env->GetMethodID(jni_helper_java_class_, "openBitmap",
            "(Ljava/lang/String;Z)Landroid/graphics/Bitmap;");

    jobject png = env->CallObjectMethod(jni_helper_java_ref_, mid, name, true);
    env->DeleteLocalRef(name);
    env->NewGlobalRef(png);

    /* Get image dimensions */
    mid = env->GetMethodID(jni_helper_java_class_, "getBitmapWidth", "(Landroid/graphics/Bitmap;)I");
    int width = env->CallIntMethod(jni_helper_java_ref_, mid, png);
    mid = env->GetMethodID(jni_helper_java_class_, "getBitmapHeight", "(Landroid/graphics/Bitmap;)I");
    int height = env->CallIntMethod(jni_helper_java_ref_, mid, png);

    /* Get pixels */
    jintArray array = env->NewIntArray(width * height);
    env->NewGlobalRef(array);
    mid = env->GetMethodID(jni_helper_java_class_, "getBitmapPixels", "(Landroid/graphics/Bitmap;[I)V");
    env->CallVoidMethod(jni_helper_java_ref_, mid, png, array);

    jint *pixels = env->GetIntArrayElements(array, 0);

    LOGI( "Loaded bitmap %s, width %d height %d",file_name, width, height);
    ...
    /* Free image */
    mid = env->GetMethodID(jni_helper_java_class_, "closeBitmap", "(Landroid/graphics/Bitmap;)V");
    env->CallVoidMethod(jni_helper_java_ref_, mid, png);
