...
def find_all_installed_files(install_dir):
    ...
    for f in sorted(installed_files.keys()):
        logging.debug(f)
    logging.debug('')
    logging.debug('found %d installed files' % len(installed_files))
    logging.debug('')
    return installed_files


def find_files_in_props(target_arch, arch_install_dir, variation, name, props, file_to_info):
    logging.debug(target_arch, arch_install_dir, variation, name, props)
...
def find_all_props_files(install_dir):
    ...
    for f in sorted(file_to_info.keys()):
        logging.debug(f)
    logging.debug('')
    logging.debug('found %d props files' % len(file_to_info))
    logging.debug('')
    return file_to_info
...
def get_ninja_inputs(ninja_binary, ninja_build_file, modules):
    ...
    logging.debug('invoke ninja', cmd)
...
def check_module_usage(install_dir, ninja_binary, ninja_file, goals):
    ...
    logging.debug('found %d ninja_inputs for goals' % len(ninja_inputs), goals)
