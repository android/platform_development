...
# convert .bp propc dictionary to .bp prop string
def gen_bp_prop(prop, ind):
    ...
    for key in prop:
        ...
        if type(val) == bool:
            ...
            prop_str += '[\n'
            for elem in val:
                prop_str += ind + INDENT + '"%s",\n' % elem
            prop_str += ind + '],\n'
        elif type(val) == dict:
            if len(val) == 0:
                continue
            prop_str += '{\n'
            prop_str += gen_bp_prop(val, ind + INDENT)
            prop_str += ind + '},\n'
        else:
...
# Remove non-existent paths from given list. Emits warning for such paths.
def filter_invalid_paths(paths, bp_dir, module_name):
    ret = []
    for path in paths:
        if os.path.exists(os.path.join(bp_dir, path)):
            ret.append(path)
        else:
            logging.warning(
                'Path "%s" of module "%s" does not exist' % (path, module_name))
    return ret
...
def convert_json_to_bp_prop(json_path, bp_dir):
    ...
    for key in prop:
        ...
        if key == 'ExportedDirs' or key == 'ExportedSystemDirs':
            val = filter_invalid_paths(
                ['include/' + x for x in val], bp_dir, module_name)
        elif key == 'InitRc' or key == 'VintfFragments':
            val = filter_invalid_paths(
                ['configs/' + x for x in val], bp_dir, module_name)

        if key in json_to_bp_map:
            ret[json_to_bp_map[key]] = val
        else:
            logging.warning(
...
def gen_bp_module(variation, name, version, target_arch, arch_props, bp_dir):
    ...
    # Forcing srcs to be arch_props prevents 32-bit only modules to be used as
    # 64-bit modules, and vice versa.
    if 'srcs' in common_prop:
        del common_prop['srcs']
