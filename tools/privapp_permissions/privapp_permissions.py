...
"""
device_side_privapp_permissions.py is a port of privapp_permissions.py that
supports generating priv-app permissions for any given device, rather than
the current built and lunched target.
"""
...
class Resources(object):
    ...
    def __init__(self, adb_path=None, aapt_path=None, use_device=None,
        ...
        use_device = use_device or serial or \
                     (apk and apk.startswith(DEVICE_PREFIX))
        ...
        self.privapp_apks = self._resolve_apks(apk)
    ...
    def _resolve_adb(adb_path):
        ...
        Returns:
            An Adb object.
        Raises:
            DependencyError if adb cannot be resolved.
        """
        if adb_path:
            if os.path.isfile(adb_path):
                adb = adb_path
            else:
                raise DependencyError('Cannot resolve adb: No such file "%s" '
                                      'exists.' % adb_path)
        else:
            try:
    ...
    def _resolve_aapt(aapt_path):
        ...
        if aapt_path:
            if os.path.isfile(aapt_path):
                ...
                raise DependencyError('Cannot resolve aapt: No such file %s '
                                      'exists.' % aapt_path)
        else:
            try:
    ...
    def _resolve_serial(self, device, serial):
        ...
        Raises:
            DependencyError if the serial trying to be returned is not usable.
        ...
        if device:
            if serial:
                ...
                try:
                    ...
                    raise DependencyError('Device with serial $ANDROID_SERIAL '
                                          '("%s") not found.' % serial)
                if 'device' in output:
                    return serial
                raise DependencyError(
    ...
    def _resolve_apks(self, apk):
        ...
        Throws:
            DependencyError if the specified apk or system/priv-app cannot be
            found.
        """
        if apk is None:
            return self._resolve_all_privapps()
        if self.adb.serial and apk.startswith(DEVICE_PREFIX):
            device_apk = apk[len(DEVICE_PREFIX):]
            try:
                return [self.adb.pull(device_apk)]
            except subprocess.CalledProcessError:
                raise DependencyError(
                    'File "%s" could not be located on device "%s".' %
                    (device_apk, self.adb.serial))
        elif apk.startswith(DEVICE_PREFIX) and self.adb.serial is None:
            raise DependencyError('Tried to resolve a file on a device without'
                                  'the --device or --serial flag.')
        elif os.path.isfile(apk):
            return [apk]
        else:
            raise DependencyError('File "%s" does not exist.' % apk)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='The serial of the device to generate permissions for. If no '
             ...
             'adb. If multiple device are found, it will default to '
    ...
    parser.add_argument(
        ...
        nargs='?',
        type=str,
        help='The local path to the APK to generate permissions for. To make '
             'the apk path device-side, prefix the path with "device:".'
    )
