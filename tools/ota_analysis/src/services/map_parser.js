 ...
 * several file name maps (physical adress: file name) according to it.
...
export class MapParser {
   ...
   */
  constructor(file) {
    this.build = new zip.ZipReader(new zip.BlobReader(file))
    this.maps = new Map()
  }
  ...
  async add(partitionName, totalLength) {
    ...
    if (this.mapFiles.get(partitionName)) {
      ...
      // Followed by the block adress, either a number or a range, for example:
  ...
  query(partitionName, extents) {
    ...
    for (let ext of extents) {
      names.push(queryMap(map, ext.startBlock))
    }
    return names
  }
}
...
/**
 * Query the hashtable <map> using index <adress>.
 * @param {Array<String>} map
 * @param {Number} adress
 */
function queryMap(map, adress) {
  return map[adress]
}