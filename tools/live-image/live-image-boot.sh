...
source $(dirname $(readlink -f "$0"))/live-image-utils.sh
...
if [ -z ${file} ]; then
    ...
    echo "live image cache file: ${cache}"
    raw=${file%.*}
    if ! [ -f ${cache} ]; then
        run_command "wget ${BUCKET_URI}/${file} -O ${cache}" "fetch..."
    fi
    if ! [ -f ${raw} ]; then
        gunzip -c ${cache} > ${raw}
    fi
...
adb shell sync
run_command "adb shell \"vdc --wait checkpoint startCheckpoint 2\"" "# start checking point"
