...
pkg=$(echo $pkgstatement | sed -re 's/package (.*);/\1/')
...
function collectImports() {
    ...
    for arg in $args; do
       if [[ $arg =~ [a-zA-Z0-9]+[.][a-zA-Z0-9]+ ]]; then
           # Ignore imports from the same package or the linter will complain.
           import=$arg                  # android.net.LinkProperties
           nopkgimport=${import#$pkg.}  # LinkProperties
           if [[ $nopkgimport =~ [.] ]]; then
...
echo "    // CHECKSTYLE:OFF Generated code"
echo
...
echo "    // CHECKSTYLE:ON Generated code"
