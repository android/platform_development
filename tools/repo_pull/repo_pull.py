...
# Shell language simple word pattern.  If a word matches this pattern, it
# doesn't have to be quoted.
_SHELL_SIMPLE_PATTERN = re.compile('^[a-zA-Z90-9_./-]+$')


def _sh_quote(word):
    """Quote a word if it contains special characters."""
    return word if _SHELL_SIMPLE_PATTERN.match(word) else json.dumps(word)
