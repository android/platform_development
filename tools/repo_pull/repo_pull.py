...
def find_android_top(curdir):
    """Find the root of Android source tree."""
    olddir = None
    while curdir != olddir:
        if os.path.exists(os.path.join(curdir, ".repo", "repo")):
            return curdir
        olddir = curdir
        curdir = os.path.dirname(curdir)
    raise ValueError("not within an Android tree")
...
def _main_bash(args):
    ...
    android_top = find_android_top(os.getcwd())
    ...
    print(_sh_quote_command(["cd", android_top]))
...
def _main_pull(args):
    ...
    os.chdir(find_android_top(os.getcwd()))
    project_dirs = build_project_name_dir_dict(args.manifest)
    branch_name = _get_local_branch_name_from_args(args)
