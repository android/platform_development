...
def main():
    ...
    for change in change_lists:
        ...
        if args.add_hashtag or args.remove_hashtag:
            _process(change, set_hashtags, url_opener, args.gerrit,
                     change['id'], args.add_hashtag, args.remove_hashtag)
        if args.set_topic:
            _process(change, set_topic, url_opener, args.gerrit, change['id'],
                     args.set_topic)
        if args.delete_topic:
            _process(change, delete_topic, url_opener, args.gerrit,
                     change['id'], expected_http_code=204)
        if args.abandon:
