...
def set_review(gerrit, change_id, labels, message):
    """Set review votes to a change list."""

    url = '{}/changes/{}/revisions/current/review'.format(gerrit, change_id)
    ...
    response_file = opener.open(url, data=json.dumps(data), header=header,
                                method='POST')
    try:
...
def main():
    ...
    # Confirm
    try:
        if input('Do you want to continue? [yn]').lower() not in {'y', 'yes'}:
            sys.exit(1)
    ...
    for change in change_lists:
        res_code, res_json = set_review(
            args.gerrit, change['id'], labels, args.message)
        ...
        if res_code != 200:
            ...
            json.dump(res_json, sys.stderr, indent=4, separators=(', ', ': '))
            print(file=sys.stderr)
            print(_SEP_SPLIT, file=sys.stderr)
