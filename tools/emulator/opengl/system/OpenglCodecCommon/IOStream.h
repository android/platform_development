...
public:
    ...
    virtual ~IOStream() {
        ...
        // It is not safe to call the virtual commitBuffer during destruction,
        // so we just log a warning if the stream is not empty
        if (m_buf && (m_free < m_bufsize)) {
                ERR("IOStream destructed while having unflushed content !!\n");
        }
    }
    ...
    unsigned char *alloc(size_t len) {
        ...
        if (!m_buf || len > m_bufsize) {
            ...
            if (!m_buf) {
                ERR("Alloc requested buffer of size %u, which is larger the the maximum buffer size %u\n", (unsigned int)len, (unsigned int)m_bufsize);
                return NULL;
            }
