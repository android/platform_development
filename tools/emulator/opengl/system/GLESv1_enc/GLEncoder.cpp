...
void GLEncoder::s_glDrawElements(void *self, GLenum mode, GLsizei count, GLenum type, void *indices)
{
    ...
    bool has_immidate_arrays = false;
    ...
    for (int i = 0; i < GLClientState::LAST_LOCATION; i++) {
        ...
        if (state->enabled) {
            if (state->bufferObject != 0) {
                ...
                has_immidate_arrays = true;
            }
        }
    }

    if (!has_immidate_arrays && !has_indirect_arrays) {
        LOGE("glDrawElements: no data bound to the command - ignoring\n");
        return;
    }

    if (ctx->m_state->currentIndexVbo() != 0) {
        if (!has_immidate_arrays) {
...
GLEncoder::~GLEncoder()
{
    if (m_compressedTextureFormats != NULL) delete m_compressedTextureFormats;
}
