                        ...
                        void** vaddr)
{
    ...
    bool hw_read_allowed = (cb->usage & GRALLOC_USAGE_HW_TEXTURE);
    bool hw_write_allowed = (cb->usage & GRALLOC_USAGE_HW_RENDER);

    if ( (sw_read && !sw_read_allowed) ||
         (sw_write && !sw_write_allowed) ||
         (hw_read && !hw_read_allowed) ||
         (hw_write && !hw_write_allowed) ) {
    ...
    if (cb->hostHandle) {
        ...
        if (sw_read || sw_write) {
            //
            // if color buffer on host is more up-to-date
            // we need to copy the buffer content from host
            //
            if (hostSyncStatus > 0) {
                if (w < cb->width || h < cb->height) {
                    int bpp = glUtilsPixelBitSize(cb->glFormat, GL_UNSIGNED_BYTE) >> 3;
                    char *tmpBuf = new char[w*h*bpp];
                    ...
                    int src_line_len = w * bpp;
                    int dst_line_len = cb->width * bpp;
                    char *dst = (char *)cpu_addr + t*dst_line_len + l*bpp;
                    char *src = tmpBuf;
                    for (int y=0; y<h; y++) {
                        memcpy(dst, src, src_line_len);
                        src += src_line_len;
                        dst += dst_line_len;
                    }
    ...
    if (sw_write) {
        ...
        // XXX: We assume here that the unlock will be called from the same process
        //      otherwise this info will not be available during unlock !!!
        //      Is this OK???
        cb->lockedLeft = l;
        cb->lockedTop = t;
        cb->lockedWidth = w;
                               ...
                               hw_device_t** device)
{
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_GPU0)) {
        ...
        dev->lock = PTHREAD_MUTEX_INITIALIZER;
