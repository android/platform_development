...
int main(int argc, char *argv[])
{
    ...
    for (int i=1; i<argc; i++) {
        if (!strncmp(argv[i], "-windowid", 9)) {
            if (sscanf(argv[++i],"%d", &iWindowId) != 1) {
                printUsage(argv[0]);
            }
        }
        else if (!strncmp(argv[i], "-port", 5)) {
            if (sscanf(argv[++i],"%d", &portNum) != 1) {
                printUsage(argv[0]);
            }
        }
        else if (!strncmp(argv[i], "-x", 2)) {
            if (sscanf(argv[++i],"%d", &winX) != 1) {
                printUsage(argv[0]);
            }
        }
        else if (!strncmp(argv[i], "-y", 2)) {
            if (sscanf(argv[++i],"%d", &winY) != 1) {
                printUsage(argv[0]);
            }
            i++;
        }
        else if (!strncmp(argv[i], "-width", 6)) {
            if (sscanf(argv[++i],"%d", &winWidth) != 1) {
                printUsage(argv[0]);
            }
        }
        else if (!strncmp(argv[i], "-height", 7)) {
            if (sscanf(argv[++i],"%d", &winHeight) != 1) {
                printUsage(argv[0]);
            }
        }
    }
