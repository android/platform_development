...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        adb_cmd = result.adb_cmd or "adb"

        # Try to run the specified adb command
        try:
            subprocess.check_output([adb_cmd, "version"],
                                    stderr=subprocess.STDOUT)
        except (OSError, subprocess.CalledProcessError):
            msg = "ERROR: Unable to run adb executable (tried '{}')."
            if not result.adb_cmd:
                msg += "\n       Try specifying its location with --adb."
            sys.exit(msg.format(adb_cmd))

        try:
            if result.serial is not None:
                result.device = adb.get_device(result.serial, adb_cmd=adb_cmd)
            elif "device" not in result or result.device == "-a":
                result.device = adb.get_device(adb_cmd=adb_cmd)
            elif result.device == "-d":
                result.device = adb.get_usb_device(adb_cmd=adb_cmd)
            elif result.device == "-e":
                result.device = adb.get_emulator_device(adb_cmd=adb_cmd)
        except (adb.DeviceNotFoundError, adb.NoUniqueDeviceError):
            # Only immediately fail if a serial was explicitly specified.
            if result.serial is not None:
                sys.exit("ERROR: Failed to find device '{}'.".format(result.serial))
            result.device = None
