...
# Helper functions used by both gdbclient.py and ndk-gdb.py.
...
def start_gdbserver(device, gdbserver_local_path, gdbserver_remote_path,
    ...
    if target_pid is not None:
        gdbserver_cmd += ["--attach", `target_pid`]
    else:
    ...
    gdbserver_cmd = run_as(user, gdbserver_cmd)
    ...
    return device.shell_direct(gdbserver_cmd, stdout=gdbclient_output,
                               stderr=gdbclient_output)


def pull_file(device, path, user=None):
    cmd = run_as(user, ["cat", path])
    stdout, _ = device.shell(cmd)
    return stdout
